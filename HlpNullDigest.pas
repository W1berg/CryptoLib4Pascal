...
begin
  raise ENotImplementedHashLibException.CreateResFmt(@SBlockSizeNotImplemented, [name]);
end;

function TNullDigest.GetHashSize: Int32;
begin
  raise ENotImplementedHashLibException.CreateResFmt(@SHashSizeNotImplemented, [name]);
end;
...
procedure TNullDigest.TransformBytes(const AData: THashLibByteArray; AIndex, ALength: Int32);
begin
  if AData <> nil then
  begin
