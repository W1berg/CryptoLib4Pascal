...
type
  ...
  class var
    ...
    class function GetCurveByName(const AName: string): IX9ECParameters; static; inline;
    class constructor UsageExamples();

    class procedure DoSigningAndVerifying(const PublicKey: IECPublicKeyParameters; const PrivateKey: IECPrivateKeyParameters; const CallerMethod, TextToSign: string; const SigningAlgo: string = SigningAlgorithmECDSA); static;

    class function EVP_GetSalt(): TBytes; static; inline;
    class function EVP_GetKeyIV(PasswordBytes, SaltBytes: TBytes; out KeyBytes, IVBytes: TBytes): Boolean; static;
    class function AES256CBCPascalCoinEncrypt(PlainText, PasswordBytes: TBytes): TBytes; static;

    class function AES256CBCPascalCoinDecrypt(CipherText, PasswordBytes: TBytes; out PlainText: TBytes): Boolean; static;
    ...
    class function ECIESPascalCoinEncrypt(const PublicKey: IAsymmetricKeyParameter; PlainText: TBytes): TBytes; static;
    class function ECIESPascalCoinDecrypt(const PrivateKey: IAsymmetricKeyParameter; CipherText: TBytes; out PlainText: TBytes): Boolean; static;
  ...
  public
    ...
    class procedure BinaryCompatiblePascalCoinAES256EncryptDecryptDemo(const inputmessage, password: string); static;
    class procedure BinaryCompatiblePascalCoinECIESEncryptDecryptDemo(const input: string); static;

    class procedure BinaryCompatiblePascalCoinECIESEncryptExistingPayloadDemo(const PublicKeyInHex, PureMessageInHex, ACurveName: string); static;

    class procedure BinaryCompatiblePascalCoinECIESDecryptExistingPayloadDemo(const PrivateKeyInHex, EncryptedMessageInHex, ACurveName: string); static;
  end;
...
class function TUsageExamples.GetCurveByName(const AName: string): IX9ECParameters;
begin
  // result := TSecNamedCurves.GetByName(AName);
  result := TCustomNamedCurves.GetByName(AName);
end;

class function TUsageExamples.ECIESPascalCoinDecrypt(const PrivateKey: IAsymmetricKeyParameter; CipherText: TBytes; out PlainText: TBytes): Boolean;
var
...
class function TUsageExamples.ECIESPascalCoinEncrypt(const PublicKey: IAsymmetricKeyParameter; PlainText: TBytes): TBytes;
var
...
class function TUsageExamples.EVP_GetKeyIV(PasswordBytes, SaltBytes: TBytes; out KeyBytes, IVBytes: TBytes): Boolean;
var
  LKey, LIV: integer;
  LDigest: IDigest;
begin
...
class function TUsageExamples.AES256CBCPascalCoinDecrypt(CipherText, PasswordBytes: TBytes; out PlainText: TBytes): Boolean;
var
...
begin
  ...
  if (System.Length(CipherText) >= SALT_MAGIC_LEN) and (TArrayUtils.AreEqual(Chopped, TConverters.ConvertStringToBytes(SALT_MAGIC, TEncoding.UTF8))) then
  begin
  ...
  Count := cipher.ProcessBytes(CipherText, LSrcStart, System.Length(CipherText) - LSrcStart, Buf, LBufStart);
...
begin
  ...
  System.SetLength(Buf, System.Length(PlainText) + LBlockSize + SALT_MAGIC_LEN + PKCS5_SALT_LEN);

  LBufStart := 0;

  System.Move(TConverters.ConvertStringToBytes(SALT_MAGIC, TEncoding.UTF8)[0], Buf[LBufStart], SALT_MAGIC_LEN * System.SizeOf(Byte));
...
class procedure TUsageExamples.BinaryCompatiblePascalCoinAES256EncryptDecryptDemo(const inputmessage, password: string);
var
...
begin
  ...
  CipherText := TUsageExamples.AES256CBCPascalCoinEncrypt(PlainText, PasswordBytes);

  if TUsageExamples.AES256CBCPascalCoinDecrypt(CipherText, PasswordBytes, DecryptedCipherText) then
  begin
    if TArrayUtils.AreEqual(PlainText, DecryptedCipherText) then
    begin
      Writeln('AES_256_CBC PascalCoin Compatability Encrypt, Decrypt Was Successful ' + sLineBreak);
      Exit;
    end;

  end;

  Writeln('AES_256_CBC PascalCoin Compatability Encrypt, Decrypt Failed ' + sLineBreak);

end;

class procedure TUsageExamples.BinaryCompatiblePascalCoinECIESEncryptExistingPayloadDemo(const PublicKeyInHex, PureMessageInHex, ACurveName: string);
...
begin
  ...
  System.Assert(PayloadToEncodeBytes <> nil, 'PayloadToDecodeBytes Cannot be Nil');
  ...
  RegeneratedPublicKey := TECPublicKeyParameters.Create('ECDSA', Lcurve.Curve.DecodePoint(PublicKeyBytes), domain);

  // Do Encryption Of Payload

  EncryptedCipherText := TUsageExamples.ECIESPascalCoinEncrypt(RegeneratedPublicKey, PayloadToEncodeBytes);

  if EncryptedCipherText <> nil then
  begin

    Writeln('ECIES PascalCoin Existing Payload Compatability Encrypt Was Successful ' + sLineBreak);

    Writeln('Encrypted Payload Message Is "' + THex.Encode(EncryptedCipherText) + '"');
    Exit;

  end;

  Writeln('ECIES PascalCoin Existing Payload Compatability Encrypt Failed ' + sLineBreak);

end;

class procedure TUsageExamples.BinaryCompatiblePascalCoinECIESDecryptExistingPayloadDemo(const PrivateKeyInHex, EncryptedMessageInHex, ACurveName: string);
...
begin
  ...
  System.Assert(EncryptedMessageInHex <> '', 'EncryptedMessageInHex Cannot be Empty');
  System.Assert(ACurveName <> '', 'ACurveName Cannot be Empty');
  ...
  System.Assert(PayloadToDecodeBytes <> nil, 'PayloadToDecodeBytes Cannot be Nil');
  ...
  KeyPair := TAsymmetricCipherKeyPair.Create(RegeneratedPublicKey, RegeneratedPrivateKey);

  // Do Signing and Verifying to Assert Proper Recreation Of Public and Private Key
  DoSigningAndVerifying(KeyPair.Public as IECPublicKeyParameters, KeyPair.Private as IECPrivateKeyParameters, MethodName, 'PascalECDSA');

  // Do Decryption Of Payload

  if TUsageExamples.ECIESPascalCoinDecrypt(RegeneratedPrivateKey, PayloadToDecodeBytes, DecryptedCipherText) then
  begin

    Writeln('ECIES PascalCoin Existing Payload Compatability Decrypt Was Successful ' + sLineBreak);

    Writeln('Decrypted Payload Message Is "' + TConverters.ConvertBytesToString(DecryptedCipherText, TEncoding.UTF8) + '"');
    Exit;

  end;

  Writeln('ECIES PascalCoin Existing Payload Compatability Decrypt Failed ' + sLineBreak);

end;

class procedure TUsageExamples.BinaryCompatiblePascalCoinECIESEncryptDecryptDemo(const input: string);
var
...
begin
  ...
  CipherText := TUsageExamples.ECIESPascalCoinEncrypt(KeyPair.Public, PlainText);

  if TUsageExamples.ECIESPascalCoinDecrypt(KeyPair.Private, CipherText, DecryptedCipherText) then
  begin
    if TArrayUtils.AreEqual(PlainText, DecryptedCipherText) then
    begin
      Writeln('ECIES PascalCoin Compatability Encrypt, Decrypt Was Successful ' + sLineBreak);
      Exit;
    end;

  end;

  Writeln('ECIES PascalCoin Compatability Encrypt, Decrypt Failed ' + sLineBreak);
end;
...
class procedure TUsageExamples.DoSigningAndVerifying(const PublicKey: IECPublicKeyParameters; const PrivateKey: IECPrivateKeyParameters; const CallerMethod, TextToSign: string; const SigningAlgo: string = SigningAlgorithmECDSA);
var
  Signer: ISigner;
...
begin
  ...
  Writeln('Public Key Normalized XCoord is: ' + pubParams.Q.Normalize.AffineXCoord.ToBigInteger.ToString(16) + sLineBreak);
  Writeln('Public Key Normalized YCoord is: ' + pubParams.Q.Normalize.AffineYCoord.ToBigInteger.ToString(16) + sLineBreak);

  Writeln('Private Key D Parameter is: ' + privParams.D.ToString(16) + sLineBreak);
...
begin
  ...
  DoSigningAndVerifying(pubParams, privParams, MethodName, 'PascalECSCHNORR', SigningAlgorithmECSCHNORR);

end;

class function TUsageExamples.GetECIESPascalCoinCompatibilityEngine: IPascalCoinIESEngine;
var
...
begin
  ...
  domain := TECDomainParameters.Create(Lcurve.Curve, Lcurve.G, Lcurve.N, Lcurve.H, Lcurve.GetSeed);
  KeyPairGeneratorInstance.Init(TECKeyGenerationParameters.Create(domain, FRandom));
  result := KeyPairGeneratorInstance.GenerateKeyPair();

  DoSigningAndVerifying(result.Public as IECPublicKeyParameters, result.Private as IECPrivateKeyParameters, MethodName, 'PascalECDSA');
...
begin
  ...
  Writeln('Encoded Public Key is: ' + BytesToHexString(EncodedPublicKey) + sLineBreak);
  ...
  if CompareMem(PByte(EncodedPublicKey), PByte(RecreatedEncodedPublicKey), System.Length(EncodedPublicKey) * System.SizeOf(Byte)) then
  begin
    Writeln('Public Key Recreation From Private Key Was Successful' + sLineBreak);
  end
  else
  begin
  ...
  begin
    Writeln('Public Key Recreation Match With Original Public Key' + sLineBreak);
  end
  else
  begin
    Writeln('Public Key Recreation DOES NOT Match With Original Public Key' + sLineBreak);
  end;

  // Do Signing and Verifying to Assert Proper Recreation Of Public Key
  DoSigningAndVerifying(recreatedPubKeyParameters, privParams, MethodName, 'PascalECDSA');

  // or the easier method
  // Method Two (** Preferred **)

  recreatedPubKeyParameters := TECKeyPairGenerator.GetCorrespondingPublicKey(privParams);
  if pubParams.Equals(recreatedPubKeyParameters) then
  begin
    Writeln('Public Key Recreation Match With Original Public Key' + sLineBreak);
  end
...
begin
  ...
  RegeneratedPublicKey := TECPublicKeyParameters.Create('ECDSA', FCurve.Curve.DecodePoint(PublicKeyByteArray), domain);
  ...
  begin
    Writeln('Private Key Recreation Match With Original Private Key' + sLineBreak);
  end
  else
  begin
    Writeln('Private Key Recreation DOES NOT Match With Original Private Key' + sLineBreak);
  end;

  // Do Signing and Verifying to Assert Proper Recreation Of Public Key

  DoSigningAndVerifying(RegeneratedPublicKey, privParams, MethodName, 'PascalECDSA');

  // Do Signing and Verifying to Assert Proper Recreation Of Private Key

  DoSigningAndVerifying(pubParams, RegeneratedPrivateKey, MethodName, 'PascalECDSA');
...
begin
  ...
  Writeln('Public Key Normalized XCoord is: ' + BigXCoord.ToString(16) + sLineBreak);
  Writeln('Public Key Normalized YCoord is: ' + BigYCoord.ToString(16) + sLineBreak);
