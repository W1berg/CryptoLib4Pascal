...
constructor TDHValidationParameters.Create(const seed: TCryptoLibByteArray; counter: Int32);
begin
  inherited Create();
  if (seed = nil) then
  begin
...
function TDHValidationParameters.Equals(const other: IDHValidationParameters): Boolean;
begin
  if other = nil then
  begin
    result := False;
