...
type
  ...
  public
    constructor Create(const ACustomization: TCryptoLibByteArray; AOutputLengthInBits: UInt64);
...
procedure TKMac.BlockUpdate(const input: TCryptoLibByteArray; inOff, len: Int32);
begin
...
begin
  ...
  begin
    buf := DoFinal();
    System.Move(buf[0], output[outOff], System.Length(buf) * System.SizeOf(Byte));
  end;
...
constructor TKMac128.Create(const ACustomization: TCryptoLibByteArray; AOutputLengthInBits: UInt64);
begin
  inherited Create();
  FOutputLengthInBits := AOutputLengthInBits;
  FKMAC := THashFactory.TKMac.CreateKMAC128(nil, ACustomization, FOutputLengthInBits);
end;

{ TKMac256 }

constructor TKMac256.Create(const ACustomization: TCryptoLibByteArray; AOutputLengthInBits: UInt64);
begin
  inherited Create();
  FOutputLengthInBits := AOutputLengthInBits;
  FKMAC := THashFactory.TKMac.CreateKMAC256(nil, ACustomization, FOutputLengthInBits);
end;
