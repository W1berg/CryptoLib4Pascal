...
type
  ...
  public
    ...
    function Encode(const bytes: TSimpleBaseLibByteArray; padding: Boolean): string;
    /// <summary>
    /// Decode a Base32 encoded string into a byte array.
    /// </summary>
...
begin
  ...
  FExtendedHex := TBase32.Create(TBase32Alphabet.ExtendedHex as IBase32Alphabet);
end;
...
function TBase32.Decode(const text: TSimpleBaseLibCharArray): TSimpleBaseLibByteArray;
var
...
begin
  ...
  begin
    c := text[textLen - 1];
    if ((c <> paddingChar) and (not TUtilities.IsWhiteSpace(text[textLen - 1]))) then
    begin
      break;
    end;
    System.Dec(textLen);
...
function TBase32.Encode(const bytes: TSimpleBaseLibByteArray; padding: Boolean): string;
var
  bytesLen, outputLen, bitsLeft, currentByte, outputPad, finalOutputLen, nextBits: Int32;
  output: string;
