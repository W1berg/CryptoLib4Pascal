...
begin
  ...
  PasswordBytes := TConverters.ConvertStringToBytes(PasswordString, TEncoding.UTF8);
  generator := TPkcs5S2ParametersGenerator.Create(TDigestUtilities.GetDigest('SHA-1'));

  generator.Init(PasswordBytes, SaltBytes, 5);

  if (not AreEqual((generator.GenerateDerivedMacParameters(64) as IKeyParameter).GetKey(), DecodeHex('D1DAA78615F287E6'))) then
  begin
  ...
  PasswordString := 'All n-entities must communicate with other n-entities via n-1 entiteeheehees';
  PasswordBytes := TConverters.ConvertStringToBytes(PasswordString, TEncoding.UTF8);

  generator.Init(PasswordBytes, SaltBytes, 500);

  if (not AreEqual((generator.GenerateDerivedMacParameters(192) as IKeyParameter).GetKey(), DecodeHex('6A8970BF68C92CAEA84A8DF28510858607126380CC47AB2D'))) then
  begin
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(192) as IKeyParameter).GetKey(), DecodeHex('29AAEF810C12ECD2236BBCFB55407F9852B5573DC1C095BB'))) then
  begin
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('0C60C80F961F0E71F3A9B524AF6012062FE037A6'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA1 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('EA6C014DC72D6F8CCD1ED92ACE1D41F0D8DE8957'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA1 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('4B007901B765489ABEAD49D926F721D065A429C1'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA1 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('3D2EEC4FE41C849B80C8D83662C0E44A8B291A964CF2F07038'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA1 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('56FA6AA75548099DCC37D7F03425E0C3'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA1 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
...
begin
  ...
  generator := TPkcs5S2ParametersGenerator.Create(TDigestUtilities.GetDigest('SHA-256'));
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('120FB6CFFCF8B32C43E7225256C4F837A86548C92CCC35480805987CB70BE17B'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA256 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('AE4D0C95AF6B46D32D0ADFF928F06DD02A303F8EF3C251DFD6E2D85A95474C43'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA256 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('C5E478D59288C841AA530DB6845C4C8D962893A001CE4E11A4963873AA98134A'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA256 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('348C89DBCBD32B2F32D814B8116E84CF2B17347EBC1800181C4E2A1FB8DD53E1C635518C7DAC47E9'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA256 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
  ...
  if (not AreEqual((generator.GenerateDerivedMacParameters(dkLen) as IKeyParameter).GetKey(), DecodeHex('89B69D0516F829893C696226650A8687'))) then
  begin
    Fail(Format('PBKDF2 HMAC-SHA256 with iteration count "%u", %u bits key generation test failed', [iteration_count, dkLen]));
  end;
