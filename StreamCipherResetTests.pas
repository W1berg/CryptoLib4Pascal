...
type
  ...
  var
    ...
    procedure DoCheckReset(const cipher: IStreamCipher; const cipherParams: ICipherParameters; encrypt: Boolean; const pretext, posttext: TBytes);

    function DoMake(CipherEngine: TCipherEngine): IStreamCipher;

    function DoRandom(size: Int32): TBytes;
    procedure DoTestReset(CipherEngine: TCipherEngine; KeyLen, IVLen: Int32); overload;
    procedure DoTestReset(const cipher1, cipher2: IStreamCipher; const cipherParams: ICipherParameters); overload;
...
procedure TTestStreamCipherReset.DoCheckReset(const cipher: IStreamCipher; const cipherParams: ICipherParameters; encrypt: Boolean; const pretext, posttext: TBytes);
var
  output: TBytes;
begin
  // Do initial run
  ...
  try
    ...
    on e: Exception do
    begin
      Fail(Format('%s init did not reset: %s', [cipher.AlgorithmName, e.Message]));
    end;
  ...
  begin
    Fail(Format('%s init did not reset. Expected %s But Got %s', [cipher.AlgorithmName, EncodeHex(posttext), EncodeHex(output)]));
  end;
  ...
  try
    ...
    on e: Exception do
    begin
      Fail(Format('%s reset did not reset: ', [cipher.AlgorithmName, e.Message]));
    end;
...
function TTestStreamCipherReset.DoMake(CipherEngine: TCipherEngine): IStreamCipher;
begin
...
procedure TTestStreamCipherReset.DoTestReset(CipherEngine: TCipherEngine; KeyLen, IVLen: Int32);
begin
  DoTestReset(DoMake(CipherEngine), DoMake(CipherEngine), TParametersWithIV.Create(TKeyParameter.Create(DoRandom(KeyLen)) as IKeyParameter, DoRandom(IVLen)) as IParametersWithIV);
end;

procedure TTestStreamCipherReset.DoTestReset(const cipher1, cipher2: IStreamCipher; const cipherParams: ICipherParameters);
var
