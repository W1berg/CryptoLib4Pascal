...
resourcestring
  ...
  SDsaDigestSignerNotInitializedForSignatureGeneration = 'DSADigestSigner not Initialized for Signature Generation.';
  SDsaDigestSignerNotInitializedForVerification = 'DSADigestSigner not Initialized for Verification';
...
type
  ...
  public
    ...
    constructor Create(const dsa: IDsaExt; const digest: IDigest; const encoding: IDsaEncoding); overload;
...
procedure TDsaDigestSigner.BlockUpdate(const input: TCryptoLibByteArray; inOff, length: Int32);
begin
...
constructor TDsaDigestSigner.Create(const dsa: IDsaExt; const digest: IDigest; const encoding: IDsaEncoding);
begin
  inherited Create();
  Fdsa := dsa;
  Fdigest := digest;
...
begin
  ...
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SDsaDigestSignerNotInitializedForSignatureGeneration);
  end;
...
procedure TDsaDigestSigner.Init(forSigning: Boolean; const parameters: ICipherParameters);
var
...
function TDsaDigestSigner.VerifySignature(const signature: TCryptoLibByteArray): Boolean;
var
  hash: TCryptoLibByteArray;
  sig: TCryptoLibGenericArray<TBigInteger>;
begin
  if (FforSigning) then
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SDsaDigestSignerNotInitializedForVerification);
  end;
