...
begin
  ...
  TConverters.le32_copy(PCardinal(LBuffer), 0, PByte(result), 0, System.Length(result));
end;
...
begin
  ...
  while LIdx < 12 do
  begin
    FBelt[LIdx + 1][LIdx mod 3] := FBelt[LIdx + 1][LIdx mod 3] xor FMill[LIdx + 1];
    System.Inc(LIdx);
  end;
  ...
  while LIdx < 19 do
  begin
    LA[LIdx] := FMill[LIdx] xor (FMill[(LIdx + 1) mod 19] or not FMill[(LIdx + 2) mod 19]);
    System.Inc(LIdx);
  end;
  ...
  while LIdx < 19 do
  begin
    FMill[LIdx] := TBits.RotateRight32(LA[(7 * LIdx) mod 19], (LIdx * (LIdx + 1)) shr 1);
    System.Inc(LIdx);
  end;
  ...
  while LIdx < 19 do
  begin
    LA[LIdx] := FMill[LIdx] xor FMill[(LIdx + 1) mod 19] xor FMill[(LIdx + 4) mod 19];
    System.Inc(LIdx);
  end;
...
procedure TRadioGatun32.TransformBlock(AData: PByte; ADataLength: Int32; AIndex: Int32);
var
