...
type
  ...
  var
    ...
    function CalculateE(const n: TBigInteger; const &message: TCryptoLibByteArray): TBigInteger; virtual;

    function CreateBasePointMultiplier(): IECMultiplier; virtual;

    function GetDenominator(coordinateSystem: Int32; const p: IECPoint): IECFieldElement; virtual;
  ...
  public
    ...
    procedure Init(forSigning: Boolean; const parameters: ICipherParameters); virtual;
    ...
    // */
    function VerifySignature(const &message: TCryptoLibByteArray; const r, s: TBigInteger): Boolean;
...
function TECDsaSigner.CalculateE(const n: TBigInteger; const &message: TCryptoLibByteArray): TBigInteger;
var
...
function TECDsaSigner.GenerateSignature(const &message: TCryptoLibByteArray): TCryptoLibGenericArray<TBigInteger>;
var
...
function TECDsaSigner.GetDenominator(coordinateSystem: Int32; const p: IECPoint): IECFieldElement;
begin
  case (coordinateSystem) of
    TECCurveConstants.COORD_HOMOGENEOUS, TECCurveConstants.COORD_LAMBDA_PROJECTIVE, TECCurveConstants.COORD_SKEWED:
      begin
...
procedure TECDsaSigner.Init(forSigning: Boolean; const parameters: ICipherParameters);
var
...
begin
  ...
  Frandom := InitSecureRandom((forSigning) and (not FkCalculator.IsDeterministic), providedRandom);
end;

function TECDsaSigner.InitSecureRandom(needed: Boolean; const provided: ISecureRandom): ISecureRandom;
begin
  if (not needed) then
  begin
    Result := nil;
...
function TECDsaSigner.VerifySignature(const &message: TCryptoLibByteArray; const r, s: TBigInteger): Boolean;
var
...
begin
  ...
  if ((Smallr.SignValue < 1) or (s.SignValue < 1) or (Smallr.CompareTo(n) >= 0) or (s.CompareTo(n) >= 0)) then
  begin
    Result := false;
    Exit;
  end;
