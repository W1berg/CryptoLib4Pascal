...
constructor TKeyGenerationParameters.Create(const Random: ISecureRandom; Strength: Int32);
begin
  if (Random = nil) then
    raise EArgumentNilCryptoLibException.CreateRes(@SRandomNil);

  if (Strength < 1) then
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidStrength, [Strength]);
