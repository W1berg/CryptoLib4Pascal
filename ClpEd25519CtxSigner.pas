...
resourcestring
  SNotInitializedForSigning = 'Ed25519CtxSigner not Initialised for Signature Generation.';
  SNotInitializedForVerifying = 'Ed25519CtxSigner not Initialised for Verification';
...
procedure TEd25519CtxSigner.BlockUpdate(const buf: TCryptoLibByteArray; off, len: Int32);
begin
  if buf <> nil then
  begin
...
constructor TEd25519CtxSigner.Create(const Ed25519Instance: IEd25519; const context: TCryptoLibByteArray);
begin
  inherited Create();
...
procedure TEd25519CtxSigner.Init(forSigning: Boolean; const parameters: ICipherParameters);
begin
...
begin
  ...
  FPrivateKey.Sign(TEd25519.TEd25519Algorithm.Ed25519ctx, FPublicKey, FContext, buf, 0, count, signature, 0);
  Reset();
  Result := signature;
end;

function TEd25519CtxSigner.VerifySignature(const signature: TCryptoLibByteArray): Boolean;
var
...
begin
  ...
  buf := Aggregate();
  count := System.Length(buf);
  Result := FEd25519Instance.Verify(signature, 0, pk, 0, FContext, buf, 0, count);
  Reset();
end;
