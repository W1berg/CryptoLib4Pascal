...
type
  TDsaPrivateKeyParameters = class sealed(TDsaKeyParameters, IDsaPrivateKeyParameters)
  ...
  public
    ...
    function Equals(const other: IDsaPrivateKeyParameters): Boolean; reintroduce; overload;
...
class function TDsaPrivateKeyParameters.Validate(const x: TBigInteger): TBigInteger;
begin
...
constructor TDsaPrivateKeyParameters.Create(const x: TBigInteger; const parameters: IDsaParameters);
begin
  inherited Create(true, parameters);
  Fx := Validate(x);
end;

function TDsaPrivateKeyParameters.Equals(const other: IDsaPrivateKeyParameters): Boolean;
begin
  if other = nil then
  begin
    result := False;
