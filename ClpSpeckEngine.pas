...
resourcestring
  ...
  SSpeck32InvalidKeySize = 'Speck32 requires a key of 64 bits but input was "%d" bits.';
  SSpeck48InvalidKeySize = 'Speck48 requires a key of 72 or 96 bits but input was "%d" bits.';
  SSpeck64InvalidKeySize = 'Speck64 requires a key of 96 or 128 bits but input was "%d" bits.';
  SSpeck96InvalidKeySize = 'Speck96 requires a key of 96 or 144 bits but input was "%d" bits.';
  SSpeck128InvalidKeySize = 'Speck128 requires a key of 128, 192 or 256 bits but input was "%d" bits.';
...
procedure TSpeckEngine.EngineInit(forEncryption: Boolean; const keyBytes: TCryptoLibByteArray);
begin
...
procedure TSpeckEngine.Init(forEncryption: Boolean; const parameters: ICipherParameters);
var
...
begin
  ...
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidParameterSpeckInit, [(parameters as TObject).ToString]);
  end;
  EngineInit(forEncryption, keyParameter.GetKey());
end;

function TSpeckEngine.ProcessBlock(const input: TCryptoLibByteArray; inOff: Int32; const output: TCryptoLibByteArray; outOff: Int32): Int32;
begin
  if (not Finitialised) then
  begin
    raise EInvalidOperationCryptoLibException.CreateResFmt(@SSpeckEngineNotInitialised, [AlgorithmName]);
  end;
...
function TSpeckUInt32Engine.BytesToWord(const bytes: TCryptoLibByteArray; off: Int32): UInt32;
var
  index: Int32;
begin
...
procedure TSpeckUInt32Engine.WordToBytes(word: UInt32; const bytes: TCryptoLibByteArray; off: Int32);
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
procedure TSpeckUInt32Engine.PackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
  WordToBytes(Fx, bytes, off + FwordSize);
  WordToBytes(Fy, bytes, off);
end;

procedure TSpeckUInt32Engine.UnPackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
...
begin
  ...
  begin
    L[(keyWords - 2) - i] := BytesToWord(keyBytes, ((keyWords - 1) - i) * FwordSize);
  end;
...
function TSpeckUInt64Engine.BytesToWord(const bytes: TCryptoLibByteArray; off: Int32): UInt64;
var
  index: Int32;
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
procedure TSpeckUInt64Engine.WordToBytes(word: UInt64; const bytes: TCryptoLibByteArray; off: Int32);
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
procedure TSpeckUInt64Engine.PackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
  WordToBytes(Fx, bytes, off + FwordSize);
  WordToBytes(Fy, bytes, off);
end;

procedure TSpeckUInt64Engine.UnPackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
  Fx := BytesToWord(bytes, off + FwordSize);
  Fy := BytesToWord(bytes, off);
end;
...
begin
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck32InvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
begin
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck48InvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
begin
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck64InvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
begin
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck96InvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
begin
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck128InvalidKeySize, [keyBytesSize * 8]);
  end;
end;
