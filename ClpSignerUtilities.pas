...
type
  ...
  public
    ...
    class function GetObjectIdentifier(mechanism: string): IDerObjectIdentifier; static; inline;

    class function GetEncodingName(const oid: IDerObjectIdentifier): string; static; inline;

    class function GetSigner(const id: IDerObjectIdentifier): ISigner; overload; static; inline;

    class function GetSigner(algorithm: string): ISigner; overload; static;

    class function InitSigner(const algorithm: string; forSigning: Boolean; const privateKey: IAsymmetricKeyParameter; const random: ISecureRandom): ISigner; overload; static; inline;

    class function InitSigner(const algorithmOid: IDerObjectIdentifier; forSigning: Boolean; const privateKey: IAsymmetricKeyParameter; const random: ISecureRandom): ISigner; overload; static; inline;
...
begin
  ...
  Falgorithms.Add(TTeleTrusTObjectIdentifiers.ECSignWithSha1.id, 'SHA-1withECDSA');
  ...
  Falgorithms.Add(TTeleTrusTObjectIdentifiers.ECSignWithRipeMD160.id, 'RIPEMD160withECDSA');
  ...
  Falgorithms.Add(TEacObjectIdentifiers.id_TA_ECDSA_SHA_1.id, 'SHA-1withCVC-ECDSA');
  ...
  Falgorithms.Add(TEacObjectIdentifiers.id_TA_ECDSA_SHA_224.id, 'SHA-224withCVC-ECDSA');
  ...
  Falgorithms.Add(TEacObjectIdentifiers.id_TA_ECDSA_SHA_256.id, 'SHA-256withCVC-ECDSA');
  ...
  Falgorithms.Add(TEacObjectIdentifiers.id_TA_ECDSA_SHA_384.id, 'SHA-384withCVC-ECDSA');
  ...
  Falgorithms.Add(TEacObjectIdentifiers.id_TA_ECDSA_SHA_512.id, 'SHA-512withCVC-ECDSA');
  ...
  Falgorithms.Add(TBsiObjectIdentifiers.ecdsa_plain_SHA1.id, 'SHA-1withPLAIN-ECDSA');
  ...
  Falgorithms.Add(TBsiObjectIdentifiers.ecdsa_plain_SHA224.id, 'SHA-224withPLAIN-ECDSA');
  ...
  Falgorithms.Add(TBsiObjectIdentifiers.ecdsa_plain_SHA256.id, 'SHA-256withPLAIN-ECDSA');
  ...
  Falgorithms.Add(TBsiObjectIdentifiers.ecdsa_plain_SHA384.id, 'SHA-384withPLAIN-ECDSA');
  ...
  Falgorithms.Add(TBsiObjectIdentifiers.ecdsa_plain_SHA512.id, 'SHA-512withPLAIN-ECDSA');
  ...
  Falgorithms.Add(TBsiObjectIdentifiers.ecdsa_plain_RIPEMD160.id, 'RIPEMD160withPLAIN-ECDSA');
  ...
  Foids.Add('RIPEMD160withECDSA', TTeleTrusTObjectIdentifiers.ECSignWithRipeMD160);
  ...
  Foids.Add('RIPEMD160withPLAIN-ECDSA', TBsiObjectIdentifiers.ecdsa_plain_RIPEMD160);
...
class function TSignerUtilities.GetEncodingName(const oid: IDerObjectIdentifier): string;
begin
  Falgorithms.TryGetValue(oid.id, Result);
end;

class function TSignerUtilities.GetObjectIdentifier(mechanism: string): IDerObjectIdentifier;
var
  aliased: string;
begin
  if (mechanism = '') then
...
class function TSignerUtilities.GetSigner(const id: IDerObjectIdentifier): ISigner;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Result := TEd25519Signer.Create(TEd25519Blake2B.Create() as IEd25519Blake2B) as IEd25519Signer;
        Exit;
      end;
      if (mechanism = 'Ed25519ctxBlake2B') then
      begin
        Result := TEd25519ctxSigner.Create(TEd25519Blake2B.Create() as IEd25519Blake2B, nil) as IEd25519ctxSigner;
        Exit;
      end;
      if (mechanism = 'Ed25519phBlake2B') then
      begin
        Result := TEd25519phSigner.Create(TEd25519Blake2B.Create() as IEd25519Blake2B, nil) as IEd25519phSigner;
        Exit;
      end;
    end
    else
    begin
      ...
      begin
        Result := TEd25519ctxSigner.Create(TEd25519.Create() as IEd25519, nil) as IEd25519ctxSigner;
        Exit;
      end;
      if (mechanism = 'Ed25519ph') then
      begin
        Result := TEd25519phSigner.Create(TEd25519.Create() as IEd25519, nil) as IEd25519phSigner;
        Exit;
      end;
    end;
  end;
  ...
  begin
    DigestName := System.Copy(mechanism, 1, TStringUtils.LastIndexOf(mechanism, 'with', True));

    DigestInstance := TDigestUtilities.GetDigest(DigestName);
    Result := (TDsaDigestSigner.Create(TDsaSigner.Create() as IDsaSigner, DigestInstance));
    Exit;
  end;
  ...
  begin
    ...
    DigestInstance := TDigestUtilities.GetDigest(DigestName);
    Result := (TDsaDigestSigner.Create(TECDsaSigner.Create() as IECDsaSigner, DigestInstance));
    Exit;
  end;

  if (TStringUtils.EndsWith(mechanism, 'withCVC-ECDSA', True) or TStringUtils.EndsWith(mechanism, 'withPLAIN-ECDSA', True)) then
  begin
    DigestName := System.Copy(mechanism, 1, TStringUtils.LastIndexOf(mechanism, 'with', True));

    DigestInstance := TDigestUtilities.GetDigest(DigestName);
    Result := (TDsaDigestSigner.Create(TECDsaSigner.Create() as IECDsaSigner, DigestInstance, TPlainDsaEncoding.Instance));
    Exit;
  end;
  ...
  begin
    ...
    DigestInstance := TDigestUtilities.GetDigest(DigestName);
    Result := (TDsaDigestSigner.Create(TECNRSigner.Create() as IECNRSigner, DigestInstance));
    Exit;
  end;
  ...
  begin
    ...
    DigestInstance := TDigestUtilities.GetDigest(DigestName);
    Result := TSchnorrDigestSigner.Create(TECSchnorrSipaSigner.Create() as IECSchnorrSipaSigner, DigestInstance, TPlainSchnorrEncoding.Instance);
    Exit;
  end;

  raise ESecurityUtilityCryptoLibException.CreateResFmt(@SUnRecognizedAlgorithm, [algorithm]);

end;

class function TSignerUtilities.InitSigner(const algorithm: string; forSigning: Boolean; const privateKey: IAsymmetricKeyParameter; const random: ISecureRandom): ISigner;
begin
  Result := TSignerUtilities.GetSigner(algorithm);
  Result.Init(forSigning, TParameterUtilities.WithRandom(privateKey, random));
end;

class function TSignerUtilities.InitSigner(const algorithmOid: IDerObjectIdentifier; forSigning: Boolean; const privateKey: IAsymmetricKeyParameter; const random: ISecureRandom): ISigner;
begin
