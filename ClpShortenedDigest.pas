...
resourcestring
  ...
  SBaseDigestSizeInsufficient = 'BaseDigest output not large enough to support length';
...
procedure TShortenedDigest.BlockUpdate(const input: TCryptoLibByteArray; inOff, len: Int32);
begin
  FBaseDigest.BlockUpdate(input, inOff, len);
end;

function TShortenedDigest.DoFinal(const output: TCryptoLibByteArray; outOff: Int32): Int32;
var
...
begin
  result := (TShortenedDigest.Create(FBaseDigest.Clone(), FLength) as IShortenedDigest) as IDigest;
end;
