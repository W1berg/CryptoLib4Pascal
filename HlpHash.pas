...
type
  ...
  public
    ...
    procedure TransformBytes(const AData: THashLibByteArray; AIndex: Int32); overload;
    procedure TransformBytes(const AData: THashLibByteArray; AIndex: Int32; ALength: Int32); overload; virtual; abstract;
    procedure TransformUntyped(const AData; ALength: Int64);
    procedure TransformStream(const AStream: TStream; ALength: Int64 = -1);
    procedure TransformFile(const AFileName: string; AFrom: Int64 = 0; ALength: Int64 = -1);

    function TransformFinal(): IHashResult; virtual; abstract;

    function ComputeString(const AData: string; AEncoding: TEncoding): IHashResult; virtual;
    ...
    function ComputeStream(const AStream: TStream; ALength: Int64 = -1): IHashResult;
    function ComputeFile(const AFileName: string; AFrom: Int64 = 0; ALength: Int64 = -1): IHashResult;
...
function THash.ComputeString(const AData: string; AEncoding: TEncoding): IHashResult;
begin
...
function THash.ComputeStream(const AStream: TStream; ALength: Int64): IHashResult;
begin
  Initialize();
...
function THash.ComputeFile(const AFileName: string; AFrom, ALength: Int64): IHashResult;
begin
  Initialize();
...
begin
  raise ENotImplementedHashLibException.CreateResFmt(@SCloneNotImplemented, [name]);
end;
...
procedure THash.TransformString(const AData: string; const AEncoding: TEncoding);
begin
...
{$ENDIF DEBUG}
  ...
  begin
    ...
    begin
      ...
      begin
        raise EIndexOutOfRangeHashLibException.CreateRes(@SReadBeyondStreamEndError);
      end;
    end;
