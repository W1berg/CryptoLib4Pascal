...
resourcestring
{$IFDEF CRYPTOLIB_MSWINDOWS}
  SMSWIndowsCryptographyAPIGenerationError = 'An Error Occured while generating random data using MS Windows Cryptography API.';
{$ENDIF}
{$IFDEF CRYPTOLIB_APPLE}
  SAppleSecRandomCopyBytesGenerationError = 'An Error Occured while generating random data using SecRandomCopyBytes API.';
{$ENDIF}
{$IFDEF CRYPTOLIB_LINUX}
  SLinuxGetRandomError = 'An Error Occured while generating random data using getRandom API';
{$ENDIF}
{$IFDEF CRYPTOLIB_SOLARIS}
  SSolarisGetRandomError = 'An Error Occured while generating random data using getRandom API';
{$ENDIF}
{$IFDEF CRYPTOLIB_GENERIC_BSD}
  SArc4RandomBufGenerationError = 'An Error Occured while generating random data using arc4random_buf API.';
{$ENDIF}
  // {$IFDEF CRYPTOLIB_UNIX}
...
{$ENDIF}
  ...
  type
    TGetRandom = function(pbBuffer: PByte; buflen: LongWord; flags: UInt32): Int32; cdecl;
  ...
  class var
    ...
    class property IsGetRandomSupportedOnOS: Boolean read GetIsGetRandomSupportedOnOS;
...
{$IFDEF CRYPTOLIB_MSWINDOWS}
  ...
  type
    ...
    TBCryptGenRandom = function(hAlgorithm: BCRYPT_ALG_HANDLE; pbBuffer: PUCHAR; cbBuffer, dwFlags: ULONG): NTStatus; stdcall;

    TBCryptOpenAlgorithmProvider = function(phAlgorithm: PVOID; pszAlgId, pszImplementation: LPCWSTR; dwFlags: ULONG): NTStatus; stdcall;
  ...
  class var
    ...
    class function GetIsCngBCryptGenRandomSupportedOnOS(): Boolean; static; inline;

    class function IsCngBCryptGenRandomAvailable(): Boolean; static;

    class property IsCngBCryptGenRandomSupportedOnOS: Boolean read GetIsCngBCryptGenRandomSupportedOnOS;

  type

    TCryptGenRandom = function(hProv: THandle; dwLen: DWORD; pbBuffer: PByte): BOOL; stdcall;

    TCryptAcquireContextW = function(phProv: Pointer; pszContainer: LPCWSTR; pszProvider: LPCWSTR; dwProvType: DWORD; dwFlags: DWORD): BOOL; stdcall;
  ...
  class var
    ...
    class property IsCryptGenRandomSupportedOnOS: Boolean read GetIsCryptGenRandomSupportedOnOS;
  ...
  class var
    ...
    class property IsRtlGenRandomSupportedOnOS: Boolean read GetIsRtlGenRandomSupportedOnOS;

    class function GetProcedureAddress(ModuleHandle: THandle; const AProcedureName: string; var AFunctionFound: Boolean): Pointer; static;
...
{$ENDIF}
    // ================================================================//
{$IFDEF CRYPTOLIB_UNIX}
    class function dev_random_device_read(len: Int32; data: PByte): Int32; static;
{$ENDIF}
    // ================================================================//
...
function SecRandomCopyBytes(rnd: SecRandomRef; count: LongWord; bytes: PByte): Int32; cdecl; external;
...
function SecRandomCopyBytes(rnd: SecRandomRef; count: LongWord; bytes: PByte): Int32; cdecl; external libSecurity name _PU + 'SecRandomCopyBytes';

{$ENDIF}
{$ENDIF}
// ************************************************************************//
{$IFDEF CRYPTOLIB_GENERIC_BSD}
procedure arc4random_buf(bytes: PByte; count: LongWord); cdecl; external 'c' name 'arc4random_buf';
{$ENDIF}
...
class function TOSRandom.GetProcedureAddress(ModuleHandle: THandle; const AProcedureName: string; var AFunctionFound: Boolean): Pointer;
begin
...
begin
  ...
  begin
    result := True;
    FBCryptOpenAlgorithmProvider := GetProcedureAddress(ModuleHandle, 'BCryptOpenAlgorithmProvider', result);
    FBCryptCloseAlgorithmProvider := GetProcedureAddress(ModuleHandle, 'BCryptCloseAlgorithmProvider', result);
...
begin
  ...
  begin
    result := True;
    FCryptAcquireContextW := GetProcedureAddress(ModuleHandle, 'CryptAcquireContextW', result);
    FCryptReleaseContext := GetProcedureAddress(ModuleHandle, 'CryptReleaseContext', result);
    FCryptGenRandom := GetProcedureAddress(ModuleHandle, 'CryptGenRandom', result);
  end;
end;
...
begin
  ...
  begin
    // Availability: Windows XP / Server 2003 and Above
    if not FCryptAcquireContextW(@hProv, nil, nil, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT or CRYPT_SILENT) then
    begin
      result := HResultFromWin32(GetLastError);
      Exit;
    end;
  ...
  begin
    ...
    if (not BCRYPT_SUCCESS(FBCryptOpenAlgorithmProvider(@hProv, PWideChar(BCRYPT_RNG_ALGORITHM), nil, 0))) then
    begin
      result := HResultFromWin32(GetLastError);
      Exit;
    end;
    ...
    try
      if (not BCRYPT_SUCCESS(FBCryptGenRandom(hProv, PUCHAR(data), ULONG(len), 0))) then
      begin
        result := HResultFromWin32(GetLastError);
        Exit;
      end;
...
class function TOSRandom.GenRandomBytesGenericBSD(len: Int32; data: PByte): Int32;
...
{$IF DEFINED(CRYPTOLIB_MSWINDOWS)}
  ...
  begin
    raise EOSRandomCryptoLibException.CreateRes(@SMSWIndowsCryptographyAPIGenerationError);
  end;

{$ELSEIF DEFINED(CRYPTOLIB_APPLE)}
  if GenRandomBytesApple(count, PByte(data)) <> 0 then
  begin
    raise EOSRandomCryptoLibException.CreateRes(@SAppleSecRandomCopyBytesGenerationError);
  end;
