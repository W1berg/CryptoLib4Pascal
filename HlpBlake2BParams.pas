...
resourcestring
  SInvalidHashSize = 'BLAKE2B HashSize must be restricted to one of the following [1 .. 64], "%d"';
  SInvalidKeyLength = '"Key" Length Must Not Be Greater Than 64, "%d"';
  SInvalidPersonalisationLength = '"Personalisation" Length Must Be Equal To 16, "%d"';
  SInvalidSaltLength = '"Salt" Length Must Be Equal To 16, "%d"';

  SInvalidFanOutParameter = 'FanOut Value Should be Between [0 .. 255] for Blake2B';
  SInvalidMaxDepthParameter = 'MaxDepth Value Should be Between [1 .. 255] for Blake2B';
  SInvalidNodeDepthParameter = 'NodeDepth Value Should be Between [0 .. 255] for Blake2B';
  SInvalidInnerHashSizeParameter = 'InnerHashSize Value Should be Between [0 .. 64] for Blake2B';
  SInvalidNodeOffsetParameter = 'NodeOffset Value Should be Between [0 .. (2^64-1)] for Blake2B';

  STreeIncorrectInnerHashSize = 'Tree Inner Hash Size Must Not Be Greater Than 64, "%d"';
...
type
  ...
  var
    ...
    procedure ValidatePersonalisationLength(const APersonalisation: THashLibByteArray); inline;
  ...
  public
    constructor Create(AHashSize: THashSize = THashSize.hsHashSize512); overload;
    constructor Create(AHashSize: Int32); overload;
    destructor Destroy; override;
    property Personalisation: THashLibByteArray read GetPersonalisation write SetPersonalisation;
...
type
  ...
  public
    ...
    class property SequentialTreeConfig: IBlake2BTreeConfig read GetSequentialTreeConfig;
...
type
  ...
  strict private
    ...
    class procedure VerifyConfigB(const AConfig: IBlake2BConfig; const ATreeConfig: IBlake2BTreeConfig; AIsSequential: Boolean); static;

  public
    class function ConfigB(const AConfig: IBlake2BConfig; var ATreeConfig: IBlake2BTreeConfig): THashLibUInt64Array; static;
...
begin
  ...
  begin
    raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidHashSize, [AHashSize]);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidKeyLength, [KeyLength]);
    end;
  end;
end;

procedure TBlake2BConfig.ValidatePersonalisationLength(const APersonalisation: THashLibByteArray);
var
...
begin
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidPersonalisationLength, [PersonalisationLength]);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidSaltLength, [SaltLength]);
    end;
  end;
end;
...
begin
  ...
  begin
    raise EArgumentInvalidHashLibException.CreateRes(@SInvalidInnerHashSizeParameter);
  end;
end;
...
begin
  ...
  begin
    raise EArgumentInvalidHashLibException.CreateRes(@SInvalidMaxDepthParameter);
  end;
end;
...
begin
  ...
  begin
    raise EArgumentInvalidHashLibException.CreateRes(@SInvalidNodeDepthParameter);
  end;
end;
...
begin
  ...
  begin
    raise EArgumentInvalidHashLibException.CreateRes(@SInvalidNodeOffsetParameter);
  end;
end;
...
class procedure TBlake2BIvBuilder.VerifyConfigB(const AConfig: IBlake2BConfig; const ATreeConfig: IBlake2BTreeConfig; AIsSequential: Boolean);
begin

  // digest length
  if ((AConfig.HashSize <= 0) or (AConfig.HashSize > 64)) then
  begin
    raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidHashSize, [AConfig.HashSize]);
  end;
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidKeyLength, [System.Length(AConfig.Key)]);
    end;
  end;
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidPersonalisationLength, [System.Length(AConfig.Personalisation)]);
    end;
  end;
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@SInvalidSaltLength, [System.Length(AConfig.Salt)]);
    end;
  end;
  ...
  begin
    ...
    begin
      raise EArgumentOutOfRangeHashLibException.Create('treeConfig.TreeIntermediateHashSize');
    end;

    if (ATreeConfig.InnerHashSize > 64) then
    begin
      raise EArgumentOutOfRangeHashLibException.CreateResFmt(@STreeIncorrectInnerHashSize, [ATreeConfig.InnerHashSize]);
    end;
  end;

end;

class function TBlake2BIvBuilder.ConfigB(const AConfig: IBlake2BConfig; var ATreeConfig: IBlake2BTreeConfig): THashLibUInt64Array;
var
...
begin
  ...
  if AConfig.Personalisation <> nil then
  begin
    System.Move(AConfig.Personalisation[0], LBuffer[48], 16 * System.SizeOf(Byte));
  end;

  System.SetLength(Result, 8);
  TConverters.le64_copy(PByte(LBuffer), 0, PUInt64(Result), 0, System.Length(LBuffer) * System.SizeOf(Byte));
end;
