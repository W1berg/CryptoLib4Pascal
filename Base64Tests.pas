...
begin
  inherited;
  FRawData := TSimpleBaseLibStringArray.Create('', 'f', 'fo', 'foo', 'foob', 'fooba', 'foobar', '1234567890123456789012345678901234567890'

    );
  FEncodedDataBase64Default := TSimpleBaseLibStringArray.Create('', 'Zg==', 'Zm8=', 'Zm9v', 'Zm9vYg==', 'Zm9vYmE=', 'Zm9vYmFy', 'MTIzNDU2Nzg5MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MA=='
...
var
  ...
  result: string;
begin
  for Idx := System.Low(FEncodedDataBase64Default) to System.High(FEncodedDataBase64Default) do
  begin
    bytes := TBase64.DefaultNoPadding.Decode(TUtilities.TrimRight(FEncodedDataBase64Default[Idx], TSimpleBaseLibCharArray.Create('=')));
    result := TEncoding.ASCII.GetString(bytes);
    CheckEquals(FRawData[Idx], result, Format('Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    ...
    CheckTrue(TUtilities.AreArraysEqual(TBase64.Default.Decode(TBase64.Default.Encode(bytes)), bytes), Format('Encoding & Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    bytes := TEncoding.ASCII.GetBytes(FRawData[Idx]);
    CheckTrue(TUtilities.AreArraysEqual(TBase64.DefaultNoPadding.Decode(TBase64.DefaultNoPadding.Encode(bytes)), bytes), Format('Encoding & Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    bytes := TEncoding.ASCII.GetBytes(FRawData[Idx]);
    CheckTrue(TUtilities.AreArraysEqual(TBase64.FileEncoding.Decode(TBase64.FileEncoding.Encode(bytes)), bytes), Format('Encoding & Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    bytes := TEncoding.ASCII.GetBytes(FRawData[Idx]);
    CheckTrue(TUtilities.AreArraysEqual(TBase64.RegExEncoding.Decode(TBase64.RegExEncoding.Encode(bytes)), bytes), Format('Encoding & Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    bytes := TEncoding.ASCII.GetBytes(FRawData[Idx]);
    CheckTrue(TUtilities.AreArraysEqual(TBase64.UrlEncoding.Decode(TBase64.UrlEncoding.Encode(bytes)), bytes), Format('Encoding & Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    bytes := TEncoding.ASCII.GetBytes(FRawData[Idx]);
    CheckTrue(TUtilities.AreArraysEqual(TBase64.XmlEncoding.Decode(TBase64.XmlEncoding.Encode(bytes)), bytes), Format('Encoding & Decoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    ...
    CheckEquals(TUtilities.TrimRight(FEncodedDataBase64Default[Idx], TSimpleBaseLibCharArray.Create('=')), result, Format('Encoding Failed at Index %d', [Idx]));
  end;
end;
...
begin
  ...
  begin
    ...
    CheckEquals(FEncodedDataBase64Default[Idx], result, Format('Encoding Failed at Index %d', [Idx]));
  end;
end;
