...
type
  ...
  public
    ...
    procedure AddSeedMaterial(const inSeed: TCryptoLibByteArray); overload; inline;
    procedure AddSeedMaterial(rSeed: Int64); overload; inline;
    procedure NextBytes(const bytes: TCryptoLibByteArray); overload; inline;
    procedure NextBytes(const bytes: TCryptoLibByteArray; start, len: Int32); overload;
...
procedure TDigestRandomGenerator.DigestUpdate(const inSeed: TCryptoLibByteArray);
begin
  Fdigest.BlockUpdate(inSeed, 0, System.Length(inSeed));
end;

procedure TDigestRandomGenerator.DigestDoFinal(const result: TCryptoLibByteArray);
begin
...
procedure TDigestRandomGenerator.AddSeedMaterial(const inSeed: TCryptoLibByteArray);
begin
  FLock.Acquire;
  try
    DigestUpdate(inSeed);
...
procedure TDigestRandomGenerator.NextBytes(const bytes: TCryptoLibByteArray; start, len: Int32);
var
