...
type
  ...
  type
{$SCOPEDENUMS ON}
    TDigestAlgorithm = (BLAKE2B_160, BLAKE2B_256, BLAKE2B_384, BLAKE2B_512, BLAKE2S_128, BLAKE2S_160, BLAKE2S_224, BLAKE2S_256, GOST3411, GOST3411_2012_256, GOST3411_2012_512, KECCAK_224, KECCAK_256, KECCAK_288, KECCAK_384, KECCAK_512, MD2, MD4, MD5, NONE, RIPEMD128, RIPEMD160, RIPEMD256, RIPEMD320,
      SHA_1, SHA_224, SHA_256, SHA_384, SHA_512, SHA_512_224, SHA_512_256, SHA3_224, SHA3_256, SHA3_384, SHA3_512, SHAKE128, SHAKE256, TIGER, WHIRLPOOL);
{$SCOPEDENUMS OFF}
  ...
  public
    ...
    class function GetObjectIdentifier(mechanism: string): IDerObjectIdentifier; static;
    class function GetDigest(const id: IDerObjectIdentifier): IDigest; overload; static; inline;
    class function GetDigest(const algorithm: string): IDigest; overload; static;

    class function GetAlgorithmName(const oid: IDerObjectIdentifier): string; static; inline;

    class function DoFinal(const digest: IDigest): TCryptoLibByteArray; overload; static; inline;
    class function DoFinal(const digest: IDigest; const input: TCryptoLibByteArray): TCryptoLibByteArray; overload; static; inline;

    class function CalculateDigest(const algorithm: string; const input: TCryptoLibByteArray): TCryptoLibByteArray; static; inline;
...
class function TDigestUtilities.GetDigest(const id: IDerObjectIdentifier): IDigest;
begin
...
begin
  ...
  digestAlgorithm := TDigestAlgorithm(GetEnumValue(TypeInfo(TDigestAlgorithm), temp));
  ...
  case digestAlgorithm of
    ...
    TDigestAlgorithm.NONE:
      begin
        result := TDigest.Create(THashFactory.TNullDigestFactory.CreateNullDigest());
        Exit;
      end;
    ...
    begin
      raise ESecurityUtilityCryptoLibException.CreateResFmt(@SUnRecognizedDigest, [mechanism]);
    end;
...
begin
  ...
  begin
    ...
    Falgorithms.Add(TRosstandartObjectIdentifiers.id_tc26_hmac_gost_3411_12_256.id, 'HMAC-GOST3411-2012-256');
    Falgorithms.Add(TRosstandartObjectIdentifiers.id_tc26_hmac_gost_3411_12_512.id, 'HMAC-GOST3411-2012-512');
    ...
    Foids.Add('GOST3411-2012-256', TRosstandartObjectIdentifiers.id_tc26_gost_3411_12_256);
    Foids.Add('GOST3411-2012-512', TRosstandartObjectIdentifiers.id_tc26_gost_3411_12_512);
  end;
end;

class function TDigestUtilities.DoFinal(const digest: IDigest): TCryptoLibByteArray;
begin
  System.SetLength(result, digest.GetDigestSize());
  digest.DoFinal(result, 0);
end;

class function TDigestUtilities.DoFinal(const digest: IDigest; const input: TCryptoLibByteArray): TCryptoLibByteArray;
begin
  digest.BlockUpdate(input, 0, System.Length(input));
  result := DoFinal(digest);
end;

class function TDigestUtilities.CalculateDigest(const algorithm: string; const input: TCryptoLibByteArray): TCryptoLibByteArray;
var
  digest: IDigest;
begin
...
class function TDigestUtilities.GetAlgorithmName(const oid: IDerObjectIdentifier): string;
begin
  Falgorithms.TryGetValue(oid.id, result);
end;

class function TDigestUtilities.GetObjectIdentifier(mechanism: string): IDerObjectIdentifier;
var
  aliased: string;
begin
  if (mechanism = '') then
