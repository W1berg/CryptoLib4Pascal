...
resourcestring
  SNotInitializedForSigning = 'Ed25519Signer not Initialised for Signature Generation.';
  SNotInitializedForVerifying = 'Ed25519Signer not Initialised for Verification';
...
procedure TEd25519Signer.BlockUpdate(const buf: TCryptoLibByteArray; off, len: Int32);
begin
  if buf <> nil then
  begin
...
procedure TEd25519Signer.Init(forSigning: Boolean; const parameters: ICipherParameters);
begin
...
begin
  ...
  FPrivateKey.Sign(TEd25519.TEd25519Algorithm.Ed25519, FPublicKey, nil, buf, 0, count, signature, 0);
  Reset();
  Result := signature;
end;

function TEd25519Signer.VerifySignature(const signature: TCryptoLibByteArray): Boolean;
var
