...
begin
  ...
  Writeln('Finished Benchmarking Default RTL Encode Implementation' + sLineBreak);
  Writeln(Format('RTL Encode Implementation Took %d milliseconds', [(b - a)]) + sLineBreak);

  Writeln('Asserting RTL Encode = SimpleBaseLib Encode' + sLineBreak);
  for i := low(FBytesContainer) to high(FBytesContainer) do
  begin

    Assert(StringReplace(Encoded[i], sLineBreak, '', [rfReplaceAll]) = b64.Encode(FBytesContainer[i]));
  end;
  ...
  b := TThread.GetTickCount;
  Writeln('Finished Benchmarking Default RTL Decode Implementation' + sLineBreak);
  Writeln(Format('RTL Decode Implementation Took %d milliseconds', [(b - a)]) + sLineBreak);

  Writeln('Asserting RTL Decode = SimpleBaseLib Decode' + sLineBreak);
  for i := low(Encoded) to high(Encoded) do
  begin
    temp := b64.Decode(StringReplace(Encoded[i], sLineBreak, '', [rfReplaceAll]));

    Assert(CompareMem(Decoded[i], temp, System.Length(Decoded[i]) * System.SizeOf(Byte)));
  end;
  ...
  b := TThread.GetTickCount;
  Writeln('Finished Benchmarking SimpleBase4Pascal Encode Implementation' + sLineBreak);
  Writeln(Format('SimpleBase4Pascal Encode Implementation Took %d milliseconds', [(b - a)]) + sLineBreak);
  ...
  b := TThread.GetTickCount;
  Writeln('Finished Benchmarking SimpleBase4Pascal Decode Implementation' + sLineBreak);
  Writeln(Format('SimpleBase4Pascal Decode Implementation Took %d milliseconds', [(b - a)]) + sLineBreak);
...
begin
  ...
  begin
    FContainer[i] := TStringGenerator.GetRandom(RandomRange(10, 1000), Boolean(RandomRange(0, 2)));
  end;
