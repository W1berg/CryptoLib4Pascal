...
type
  ...
  const
    ...
    function CopyPoints(const ps: TCryptoLibGenericArray<IECPoint>; len: Int32): TCryptoLibGenericArray<IECPoint>;
    function CopyScalars(const ks: TCryptoLibGenericArray<TBigInteger>; len: Int32): TCryptoLibGenericArray<TBigInteger>;
    ...
    procedure AddTestCurves(x9s: TList<IX9ECParameters>; const x9: IX9ECParameters);
...
procedure TTestECAlgorithms.AddTestCurves(x9s: TList<IX9ECParameters>; const x9: IX9ECParameters);
var
  curve, c: IECCurve;
  point: IECPoint;
...
procedure TTestECAlgorithms.AssertPointsEqual(const msg: string; const a, b: IECPoint);
begin
  CheckEquals(True, a.Equals(b), msg);
end;

function TTestECAlgorithms.CopyPoints(const ps: TCryptoLibGenericArray<IECPoint>; len: Int32): TCryptoLibGenericArray<IECPoint>;
begin
  System.SetLength(Result, len);
  Result := System.Copy(ps, 0, len);
end;

function TTestECAlgorithms.CopyScalars(const ks: TCryptoLibGenericArray<TBigInteger>; len: Int32): TCryptoLibGenericArray<TBigInteger>;
begin
  System.SetLength(Result, len);
...
begin
  ...
  begin
    ...
    v := TECAlgorithms.SumOfMultiplies(CopyPoints(points, i + 1), CopyScalars(scalars, i + 1));
...
begin
  ...
  begin
    ...
    AssertPointsEqual('TECAlgorithms.ShamirsTrick is incorrect', results[0], results[1]);
    AssertPointsEqual('TECAlgorithms.SumOfTwoMultiplies is incorrect', results[0], results[2]);
...
function TTestECAlgorithms.GetRandomScalar(const x9: IX9ECParameters): TBigInteger;
begin
  Result := TBigInteger.Create(x9.N.BitLength, FRandom);
end;

function TTestECAlgorithms.GetTestCurves: TCryptoLibGenericArray<IX9ECParameters>;
var
  name: string;
