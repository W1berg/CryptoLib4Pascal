...
type
  ...
  private
    ...
    function MeasureChiSquared(const random: ISecureRandom; rounds: Int32): Double;
...
begin
  ...
  CheckEquals(true, RunChiSquaredTests(random), 'Chi2 test detected possible non-randomness');
end;

function TTestSecureRandom.MeasureChiSquared(const random: ISecureRandom; rounds: Int32): Double;
var
  opts, bs: TBytes;
...
function TTestSecureRandom.RunChiSquaredTests(const random: ISecureRandom): Boolean;
var
  passes, tries: Int32;
  chi2: Double;
begin
...
begin
  random := TSecureRandom.Create(TCryptoApiRandomGenerator.Create(TRandomNumberGenerator.CreateRNG(TRandomNumberGenerator.TRandomNumberGeneratorMode.rngmOS)) as ICryptoApiRandomGenerator);
...
var
  &random: ISecureRandom;
begin
  random := TSecureRandom.Create(TCryptoApiRandomGenerator.Create(TRandomNumberGenerator.CreateRNG(TRandomNumberGenerator.TRandomNumberGeneratorMode.rngmAES)) as ICryptoApiRandomGenerator);
...
var
  &random: ISecureRandom;
begin
  random := TSecureRandom.Create(TCryptoApiRandomGenerator.Create() as ICryptoApiRandomGenerator);
