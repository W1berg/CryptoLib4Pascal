...
type
  ...
  /// </summary>
  TPkcs5S2ParametersGenerator = class sealed(TPbeParametersGenerator, IPkcs5S2ParametersGenerator)
  ...
  public
    ...
    procedure Init(const password, salt: TCryptoLibByteArray; iterationCount: Int32);
...
function TPkcs5S2ParametersGenerator.GenerateDerivedKey(dkLen: Int32): TCryptoLibByteArray;
begin
  result := FPBKDF2_HMAC.GetBytes(dkLen);
end;

function TPkcs5S2ParametersGenerator.GenerateDerivedMacParameters(keySize: Int32): ICipherParameters;
var
...
function TPkcs5S2ParametersGenerator.GenerateDerivedParameters(const algorithm: string; keySize: Int32): ICipherParameters;
var
  dKey: TCryptoLibByteArray;
begin
  keySize := keySize div 8;
...
function TPkcs5S2ParametersGenerator.GenerateDerivedParameters(const algorithm: string; keySize, ivSize: Int32): ICipherParameters;
var
  dKey: TCryptoLibByteArray;
  key: IKeyParameter;
begin
...
procedure TPkcs5S2ParametersGenerator.Init(const password, salt: TCryptoLibByteArray; iterationCount: Int32);
begin
  FPassword := System.Copy(password);
  FSalt := System.Copy(salt);
  FPBKDF2_HMAC := TKDF.TPBKDF2_HMAC.CreatePBKDF2_HMAC(Fdigest.GetUnderlyingIHash, FPassword, FSalt, iterationCount);
end;
