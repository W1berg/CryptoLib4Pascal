...
resourcestring
  SNotInitializedForSigning = 'Ed25519PhSigner not Initialised for Signature Generation.';
  SNotInitializedForVerifying = 'Ed25519PhSigner not Initialised for Verification';
...
type
  ...
  public
    constructor Create(const Ed25519Instance: IEd25519; const context: TCryptoLibByteArray);
    destructor Destroy(); override;

    procedure Init(forSigning: Boolean; const parameters: ICipherParameters); virtual;
    procedure Update(b: Byte); virtual;
    procedure BlockUpdate(const buf: TCryptoLibByteArray; off, len: Int32); virtual;
    function GenerateSignature(): TCryptoLibByteArray; virtual;
    function VerifySignature(const signature: TCryptoLibByteArray): Boolean; virtual;
...
procedure TEd25519PhSigner.BlockUpdate(const buf: TCryptoLibByteArray; off, len: Int32);
begin
  FPreHash.BlockUpdate(buf, off, len);
end;

constructor TEd25519PhSigner.Create(const Ed25519Instance: IEd25519; const context: TCryptoLibByteArray);
begin
  inherited Create();
...
procedure TEd25519PhSigner.Init(forSigning: Boolean; const parameters: ICipherParameters);
begin
...
begin
  ...
  FPrivateKey.Sign(TEd25519.TEd25519Algorithm.Ed25519Ph, FPublicKey, FContext, msg, 0, TEd25519.PreHashSize, signature, 0);
  Result := signature;
end;

function TEd25519PhSigner.VerifySignature(const signature: TCryptoLibByteArray): Boolean;
var
...
begin
  ...
  Result := FEd25519Instance.VerifyPrehash(signature, 0, pk, 0, FContext, FPreHash);
end;
