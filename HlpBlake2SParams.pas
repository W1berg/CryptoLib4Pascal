...
resourcestring
  SInvalidHashSize = 'BLAKE2S HashSize must be restricted to one of the following [1 .. 32], "%d"';
  SInvalidKeyLength = '"Key" Length Must Not Be Greater Than 32, "%d"';
  SInvalidPersonalisationLength = '"Personalisation" Length Must Be Equal To 8, "%d"';
  SInvalidSaltLength = '"Salt" Length Must Be Equal To 8, "%d"';

  SInvalidFanOutParameter = 'FanOut Value Should be Between [0 .. 255] for Blake2S';
  SInvalidMaxDepthParameter = 'MaxDepth Value Should be Between [1 .. 255] for Blake2S';
  SInvalidNodeDepthParameter = 'NodeDepth Value Should be Between [0 .. 255] for Blake2S';
  SInvalidInnerHashSizeParameter = 'InnerHashSize Value Should be Between [0 .. 32] for Blake2S';
  SInvalidNodeOffsetParameter = 'NodeOffset Value Should be Between [0 .. (2^48-1)] for Blake2S';

  STreeIncorrectInnerHashSize = 'Tree Inner Hash Size Must Not Be Greater Than 32, "%d"';
...
type
  ...
  public
    constructor Create(AHashSize: THashSize = THashSize.hsHashSize256); overload;
...
type
  ...
  public
    ...
    class property SequentialTreeConfig: IBlake2STreeConfig read GetSequentialTreeConfig;
...
type
  ...
  strict private
    ...
    class procedure VerifyConfigS(const AConfig: IBlake2SConfig; const ATreeConfig: IBlake2STreeConfig; AIsSequential: Boolean); static;

  public
    class function ConfigS(const AConfig: IBlake2SConfig; var ATreeConfig: IBlake2STreeConfig): THashLibUInt32Array; static;
...
procedure TBlake2SConfig.ValidatePersonalisationLength(const APersonalisation: THashLibByteArray);
var
...
class procedure TBlake2SIvBuilder.VerifyConfigS(const AConfig: IBlake2SConfig; const ATreeConfig: IBlake2STreeConfig; AIsSequential: Boolean);
begin
...
class function TBlake2SIvBuilder.ConfigS(const AConfig: IBlake2SConfig; var ATreeConfig: IBlake2STreeConfig): THashLibUInt32Array;
var
...
begin
  ...
  if AConfig.Personalisation <> nil then
  begin
    System.Move(AConfig.Personalisation[0], LBuffer[24], 8 * System.SizeOf(Byte));
  end;

  System.SetLength(Result, 8);
  TConverters.le32_copy(PByte(LBuffer), 0, PCardinal(Result), 0, System.Length(LBuffer) * System.SizeOf(Byte));
end;
