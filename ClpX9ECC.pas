...
type
  ...
  public
    ...
    class function GetByteLength(const fe: IECFieldElement): Int32; overload; static; inline;
    class function GetByteLength(const c: IECCurve): Int32; overload; static; inline;

    class function IntegerToBytes(const s: TBigInteger; qLength: Int32): TCryptoLibByteArray; static;
...
type
  ...
  public
    ...
    constructor Create(const p: TBigInteger; const s: IAsn1OctetString); overload; deprecated 'Will be removed';
    constructor Create(m, k1, k2, k3: Int32; const s: IAsn1OctetString); overload; deprecated 'Will be removed';
...
type
  ...
  public
    ...
    constructor Create(const c: IECCurve; const encoding: TCryptoLibByteArray); overload;
...
type
  ...
  public
    ...
    constructor Create(const curve: IECCurve; const seed: TCryptoLibByteArray); overload;
    constructor Create(const fieldID: IX9FieldID; const seq: IAsn1Sequence); overload; deprecated 'Use constructor including order/cofactor';

    constructor Create(const fieldID: IX9FieldID; const order, cofactor: TBigInteger; const seq: IAsn1Sequence); overload;
...
class function TX9IntegerConverter.GetByteLength(const fe: IECFieldElement): Int32;
begin
...
class function TX9IntegerConverter.IntegerToBytes(const s: TBigInteger; qLength: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  begin
    ...
    System.Move(bytes[System.Length(bytes) - System.Length(tmp)], tmp[0], System.Length(tmp) * System.SizeOf(Byte));
    result := tmp;
    Exit;
  end;
  if (qLength > System.Length(bytes)) then
  begin
    System.SetLength(tmp, qLength);
    System.Move(bytes[0], tmp[System.Length(tmp) - System.Length(bytes)], System.Length(bytes) * System.SizeOf(Byte));
    result := tmp;
    Exit;
  end;
...
begin
  ...
  fieldIdParams := TAsn1EncodableVector.Create([TDerInteger.Create(m) as IDerInteger]);
  ...
  begin
    ...
    fieldIdParams.Add([TX9ObjectIdentifiers.TPBasis, TDerInteger.Create(k1) as IDerInteger]);
  end
  else
  begin
    ...
    fieldIdParams.Add([TX9ObjectIdentifiers.PPBasis, TDerSequence.Create([TDerInteger.Create(k1) as IDerInteger, TDerInteger.Create(k2) as IDerInteger, TDerInteger.Create(k3) as IDerInteger])]);
  end;
...
constructor TX9FieldElement.Create(const p: TBigInteger; const s: IAsn1OctetString);
begin
  Create(TFpFieldElement.Create(p, TBigInteger.Create(1, s.GetOctets())) as IFpFieldElement)
end;
...
constructor TX9FieldElement.Create(m, k1, k2, k3: Int32; const s: IAsn1OctetString);
begin
  Create(TF2mFieldElement.Create(m, k1, k2, k3, TBigInteger.Create(1, s.GetOctets())) as IF2mFieldElement)
end;
...
begin
  ...
  paddedBigInteger := TX9IntegerConverter.IntegerToBytes(Ff.ToBigInteger(), byteCount);
...
constructor TX9ECPoint.Create(const c: IECCurve; const encoding: TCryptoLibByteArray);
begin
  inherited Create();
  Fc := c;
...
begin
  ...
  result := (octets <> nil) and (System.Length(octets) > 0) and ((octets[0] = 2) or (octets[0] = 3));
end;
...
constructor TX9Curve.Create(const curve: IECCurve; const seed: TCryptoLibByteArray);
begin
  inherited Create();
  if (curve = nil) then
  begin
...
constructor TX9Curve.Create(const fieldID: IX9FieldID; const seq: IAsn1Sequence);
begin
...
begin
  ...
  begin
    ...
    A := TBigInteger.Create(1, TAsn1OctetString.GetInstance(seq[0] as TAsn1Encodable).GetOctets());
    B := TBigInteger.Create(1, TAsn1OctetString.GetInstance(seq[1] as TAsn1Encodable).GetOctets());
    Fcurve := TFpCurve.Create(p, A, B, order, cofactor);
  end
  else if (FfieldIdentifier.Equals(TX9ObjectIdentifiers.CharacteristicTwoField)) then
  begin
...
begin
  ...
  if (FfieldIdentifier.Equals(TX9ObjectIdentifiers.PrimeField) or FfieldIdentifier.Equals(TX9ObjectIdentifiers.CharacteristicTwoField)) then
  begin
    v.Add([(TX9FieldElement.Create(Fcurve.A) as IX9FieldElement).ToAsn1Object()]);
    v.Add([(TX9FieldElement.Create(Fcurve.B) as IX9FieldElement).ToAsn1Object()]);
  end;
