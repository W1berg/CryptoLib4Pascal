...
type
  TScalarSplitParameters = class sealed(TInterfacedObject, IScalarSplitParameters)
  ...
  strict private
    ...
    class procedure CheckVector(const v: TCryptoLibGenericArray<TBigInteger>; const name: string); static;
  ...
  public
    constructor Create(const v1, v2: TCryptoLibGenericArray<TBigInteger>; const g1, g2: TBigInteger; bits: Int32);
...
class procedure TScalarSplitParameters.CheckVector(const v: TCryptoLibGenericArray<TBigInteger>; const name: string);
begin
  if ((v = nil) or (System.length(v) <> 2) or (not v[0].IsInitialized) or (not v[1].IsInitialized)) then
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidParameters, [name]);
  end;
end;

constructor TScalarSplitParameters.Create(const v1, v2: TCryptoLibGenericArray<TBigInteger>; const g1, g2: TBigInteger; bits: Int32);
begin
  CheckVector(v1, 'v1');
  CheckVector(v2, 'v2');
