...
resourcestring
  SInvalidValueForSecP256K1FieldElement = 'Value Invalid for SecP256K1FieldElement "%s"';
...
type
  ...
  public
    ...
    class procedure Reduce32(x: UInt32; const z: TCryptoLibUInt32Array); static; inline;
...
type
  TSecP256K1FieldElement = class(TAbstractFpFieldElement, ISecP256K1FieldElement)

  strict private

    function Equals(const other: ISecP256K1FieldElement): Boolean; reintroduce; overload;
...
type
  ...
  type
    TSecP256K1LookupTable = class sealed(TAbstractECLookupTable, ISecP256K1LookupTable)
    ...
    public
      ...
      constructor Create(const outer: ISecP256K1Curve; const table: TCryptoLibUInt32Array; size: Int32);
...
begin
  FP := TCryptoLibUInt32Array.Create($FFFFFC2F, $FFFFFFFE, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF);
  FPExt := TCryptoLibUInt32Array.Create($000E90A1, $000007A2, $00000001, $00000000, $00000000, $00000000, $00000000, $00000000, $FFFFF85E, $FFFFFFFD, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF);
  FPExtInv := TCryptoLibUInt32Array.Create($FFF16F5F, $FFFFF85D, $FFFFFFFE, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $FFFFFFFF, $000007A1, $00000002);
end;

class function TSecP256K1Field.FromBigInteger(const x: TBigInteger): TCryptoLibUInt32Array;
var
...
class procedure TSecP256K1Field.MultiplyAddToExt(const x, y, zz: TCryptoLibUInt32Array);
var
  c: UInt32;
begin
...
class procedure TSecP256K1Field.Reduce32(x: UInt32; const z: TCryptoLibUInt32Array);
begin
  if (((x <> 0) and (TNat256.Mul33WordAdd(PInv33, x, z, 0) <> 0)) or ((z[7] = P7) and (TNat256.Gte(z, FP)))) then
  begin
    TNat.Add33To(8, PInv33, z);
  end;
end;
...
class procedure TSecP256K1Field.SquareN(const x: TCryptoLibUInt32Array; n: Int32; const z: TCryptoLibUInt32Array);
var
  tt: TCryptoLibUInt32Array;
begin
{$IFDEF DEBUG}
...
class procedure TSecP256K1Field.SubtractExt(const xx, yy, zz: TCryptoLibUInt32Array);
var
  c: Int32;
begin
...
begin
  result := TBigInteger.Create(1, THex.Decode('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F'));
end;
...
begin
  ...
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidValueForSecP256K1FieldElement, ['x']);
  end;
  inherited Create();
...
function TSecP256K1FieldElement.Multiply(const b: IECFieldElement): IECFieldElement;
var
  z: TCryptoLibUInt32Array;
begin
  z := TNat256.Create();
...
var
  x1, x2, x3, x6, x9, x11, x22, x44, x88, x176, x220, x223, t1, t2: TCryptoLibUInt32Array;
begin
  { *
...
function TSecP256K1FieldElement.Subtract(const b: IECFieldElement): IECFieldElement;
var
  z: TCryptoLibUInt32Array;
begin
  z := TNat256.Create();
...
function TSecP256K1FieldElement.Divide(const b: IECFieldElement): IECFieldElement;
var
  z: TCryptoLibUInt32Array;
begin
  z := TNat256.Create();
...
function TSecP256K1FieldElement.Equals(const other: ISecP256K1FieldElement): Boolean;
begin
...
constructor TSecP256K1Point.Create(const curve: IECCurve; const x, y: IECFieldElement);
begin
  Create(curve, x, y, false);
end;

constructor TSecP256K1Point.Create(const curve: IECCurve; const x, y: IECFieldElement; withCompression: Boolean);
begin
...
constructor TSecP256K1Point.Create(const curve: IECCurve; const x, y: IECFieldElement; const zs: TCryptoLibGenericArray<IECFieldElement>; withCompression: Boolean);
begin
...
begin
  ...
  result := TSecP256K1Point.Create(Lcurve, x3, Y3, zs, IsCompressed) as IECPoint;
end;
...
begin
  ...
  result := TSecP256K1Point.Create(curve, RawXCoord, RawYCoord.Negate(), RawZCoords, IsCompressed) as IECPoint;
end;
...
begin
  ...
  result := TSecP256K1Point.Create(Lcurve, x3, Y3, TCryptoLibGenericArray<IECFieldElement>.Create(Z3), IsCompressed) as IECPoint;
end;
...
begin
  ...
  Fm_order := TBigInteger.Create(1, THex.Decode('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141'));
...
function TSecP256K1Curve.CreateCacheSafeLookupTable(const points: TCryptoLibGenericArray<IECPoint>; off, len: Int32): IECLookupTable;
var
...
function TSecP256K1Curve.CreateRawPoint(const x, y: IECFieldElement; withCompression: Boolean): IECPoint;
begin
  result := TSecP256K1Point.Create(Self as IECCurve, x, y, withCompression);
end;

function TSecP256K1Curve.CreateRawPoint(const x, y: IECFieldElement; const zs: TCryptoLibGenericArray<IECFieldElement>; withCompression: Boolean): IECPoint;
begin
...
constructor TSecP256K1Curve.TSecP256K1LookupTable.Create(const outer: ISecP256K1Curve; const table: TCryptoLibUInt32Array; size: Int32);
begin
  inherited Create();
  Fm_outer := outer;
  Fm_table := table;
...
function TSecP256K1Curve.TSecP256K1LookupTable.CreatePoint(const x, y: TCryptoLibUInt32Array): IECPoint;
var
  XFieldElement, YFieldElement: ISecP256K1FieldElement;
  SECP256K1_AFFINE_ZS: TCryptoLibGenericArray<IECFieldElement>;
begin
  SECP256K1_AFFINE_ZS := TCryptoLibGenericArray<IECFieldElement>.Create(TSecP256K1FieldElement.Create(TBigInteger.One) as ISecP256K1FieldElement);

  XFieldElement := TSecP256K1FieldElement.Create(x);
  YFieldElement := TSecP256K1FieldElement.Create(y);
  result := Fm_outer.CreateRawPoint(XFieldElement, YFieldElement, SECP256K1_AFFINE_ZS, false);
end;
...
function TSecP256K1Curve.TSecP256K1LookupTable.LookupVar(index: Int32): IECPoint;
var
  x, y: TCryptoLibUInt32Array;
  pos, J: Int32;
begin
