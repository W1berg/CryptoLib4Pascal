...
resourcestring
  ...
  SInvalidParameterSpeckLegacyInit = 'Invalid Parameter Passed to SpeckLegacy Init - "%s"';
  SSpeck32LegacyInvalidKeySize = 'Speck32Legacy requires a key of 64 bits but input was "%d" bits.';
  SSpeck48LegacyInvalidKeySize = 'Speck48Legacy requires a key of 72 or 96 bits but input was "%d" bits.';
  SSpeck64LegacyInvalidKeySize = 'Speck64Legacy requires a key of 96 or 128 bits but input was "%d" bits.';
  SSpeck96LegacyInvalidKeySize = 'Speck96Legacy requires a key of 96 or 144 bits but input was "%d" bits.';
  SSpeck128LegacyInvalidKeySize = 'Speck128Legacy requires a key of 128, 192 or 256 bits but input was "%d" bits.';
...
type
  ...
  /// implementations .
  /// </summary>
  TSpeckLegacyEngine = class abstract(TInterfacedObject, ISpeckLegacyEngine, IBlockCipher)
  ...
  var
    ...
    /// </summary>
    procedure EngineInit(forEncryption: Boolean; const keyBytes: TCryptoLibByteArray); virtual;

  strict protected

  var
    FblockSize, FwordSize, FwordSizeBits, Falpha, Fbeta, FbaseRounds, Frounds: Int32;
    ...
    /// method is valid.
    /// </summary>
    procedure CheckKeySize(const keyBytes: TCryptoLibByteArray); virtual; abstract;
    ...
    /// </param>
    procedure UnPackBlock(const bytes: TCryptoLibByteArray; off: Int32); virtual; abstract;
    ...
    /// </param>
    procedure PackBlock(const bytes: TCryptoLibByteArray; off: Int32); virtual; abstract;
...
type
  ...
  var
    ...
    /// </returns>
    function BytesToWord(const bytes: TCryptoLibByteArray; off: Int32): UInt32; inline;
    ...
    /// </param>
    procedure WordToBytes(word: UInt32; const bytes: TCryptoLibByteArray; off: Int32); inline;
  ...
  strict protected
    ...
    procedure UnPackBlock(const bytes: TCryptoLibByteArray; off: Int32); override;
...
type
  ...
  var
    ...
    /// the read word, with zeroes in any bits higher than the word size.
    /// </returns>
    function BytesToWord(const bytes: TCryptoLibByteArray; off: Int32): UInt64; inline;
    ...
    /// the offset to write the data at.
    /// </param>
    procedure WordToBytes(word: UInt64; const bytes: TCryptoLibByteArray; off: Int32); inline;
...
procedure TSpeckLegacyEngine.EngineInit(forEncryption: Boolean; const keyBytes: TCryptoLibByteArray);
begin
...
procedure TSpeckLegacyEngine.Init(forEncryption: Boolean; const parameters: ICipherParameters);
var
...
begin
  ...
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidParameterSpeckLegacyInit, [(parameters as TObject).ToString]);
  end;
  EngineInit(forEncryption, keyParameter.GetKey());
end;

function TSpeckLegacyEngine.ProcessBlock(const input: TCryptoLibByteArray; inOff: Int32; const output: TCryptoLibByteArray; outOff: Int32): Int32;
begin
  if (not Finitialised) then
  begin
    raise EInvalidOperationCryptoLibException.CreateResFmt(@SSpeckLegacyEngineNotInitialised, [AlgorithmName]);
  end;

  TCheck.DataLength((inOff + FblockSize) > System.Length(input), SInputBuffertooShort);
  TCheck.DataLength((outOff + FblockSize) > System.Length(output), SOutputBuffertooShort);
...
function TSpeckUInt32LegacyEngine.BytesToWord(const bytes: TCryptoLibByteArray; off: Int32): UInt32;
var
  index: Int32;
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
procedure TSpeckUInt32LegacyEngine.WordToBytes(word: UInt32; const bytes: TCryptoLibByteArray; off: Int32);
var
  index: Int32;
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
constructor TSpeckUInt32LegacyEngine.Create(wordSize, baseRounds, alpha, beta: Int32);
begin
...
procedure TSpeckUInt32LegacyEngine.PackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
  WordToBytes(Fy, bytes, off + FwordSize);
  WordToBytes(Fx, bytes, off);
end;

procedure TSpeckUInt32LegacyEngine.UnPackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
...
function TSpeckUInt64LegacyEngine.BytesToWord(const bytes: TCryptoLibByteArray; off: Int32): UInt64;
var
  index: Int32;
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
procedure TSpeckUInt64LegacyEngine.WordToBytes(word: UInt64; const bytes: TCryptoLibByteArray; off: Int32);
var
  index: Int32;
begin
  TCheck.DataLength((off + FwordSize) > System.Length(bytes), SInvalidArgumentEncountered);
...
constructor TSpeckUInt64LegacyEngine.Create(wordSize, baseRounds, alpha, beta: Int32);
begin
  inherited Create(wordSize, baseRounds, alpha, beta);
end;
...
procedure TSpeckUInt64LegacyEngine.PackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
  WordToBytes(Fy, bytes, off + FwordSize);
  WordToBytes(Fx, bytes, off);
end;

procedure TSpeckUInt64LegacyEngine.UnPackBlock(const bytes: TCryptoLibByteArray; off: Int32);
begin
  Fy := BytesToWord(bytes, off + FwordSize);
  Fx := BytesToWord(bytes, off);
end;
...
procedure TSpeck32LegacyEngine.CheckKeySize(const keyBytes: TCryptoLibByteArray);
var
  keyBytesSize: Int32;
begin
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck32LegacyInvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
procedure TSpeck48LegacyEngine.CheckKeySize(const keyBytes: TCryptoLibByteArray);
var
  keyBytesSize: Int32;
begin
  keyBytesSize := System.Length(keyBytes);
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck48LegacyInvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
procedure TSpeck64LegacyEngine.CheckKeySize(const keyBytes: TCryptoLibByteArray);
var
  keyBytesSize: Int32;
begin
  keyBytesSize := System.Length(keyBytes);
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck64LegacyInvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
procedure TSpeck96LegacyEngine.CheckKeySize(const keyBytes: TCryptoLibByteArray);
var
  keyBytesSize: Int32;
begin
  keyBytesSize := System.Length(keyBytes);
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck96LegacyInvalidKeySize, [keyBytesSize * 8]);
  end;
end;
...
procedure TSpeck128LegacyEngine.CheckKeySize(const keyBytes: TCryptoLibByteArray);
var
  keyBytesSize: Int32;
begin
  keyBytesSize := System.Length(keyBytes);
  ...
  begin
    TArrayUtils.ZeroFill(keyBytes);
    raise EArgumentCryptoLibException.CreateResFmt(@SSpeck128LegacyInvalidKeySize, [keyBytesSize * 8]);
  end;
end;
