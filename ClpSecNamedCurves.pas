...
type
  ...
  class var
    ...
    class function ConfigureCurveGlv(const c: IECCurve; const p: IGlvTypeBParameters): IECCurve; static; inline;
...
class function TSecNamedCurves.ConfigureBasepoint(const curve: IECCurve; const encoding: string): IX9ECPoint;
begin
  result := TX9ECPoint.Create(curve, THex.Decode(encoding));
  TWnafUtilities.ConfigureBasepoint(result.Point);
end;

class procedure TSecNamedCurves.DefineCurve(const name: string; const oid: IDerObjectIdentifier; const holder: IX9ECParametersHolder);
begin
...
class function TSecNamedCurves.ConfigureCurveGlv(const c: IECCurve; const p: IGlvTypeBParameters): IECCurve;
var
...
class function TSecNamedCurves.GetByOid(const oid: IDerObjectIdentifier): IX9ECParameters;
var
...
begin
  ...
  G := ConfigureBasepoint(curve, '0409487239995A5EE76B55F9C2F098A89CE5AF8724C0A23E0E0FF77500');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '044BA30AB5E892B4E1649DD0928643ADCD46F5882E3747DEF36E956E97');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '04161FF7528B899B2D0C28607CA52C5B86CF5AC8395BAFEB13C02DA292DDED7A83');
...
begin
  ...
  glv := TGlvTypeBParameters.Create(TBigInteger.Create('9ba48cba5ebcb9b6bd33b92830b2a2e0e192f10a', 16), TBigInteger.Create('c39c6c3b3a36d7701b9c71a1f5804ae5d0003f4', 16),

    TScalarSplitParameters.Create(TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('9162fbe73984472a0a9e', 16), TBigInteger.Create('-96341f1138933bc2f505', 16)),

    TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('127971af8721782ecffa3', 16), TBigInteger.Create('9162fbe73984472a0a9e', 16)), TBigInteger.Create('9162fbe73984472a0a9d0590', 16), TBigInteger.Create('96341f1138933bc2f503fd44', 16), 176) as IScalarSplitParameters);
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '044A96B5688EF573284664698968C38BB913CBFC8223A628553168947D59DCC912042351377AC5FB32');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '0452DCB034293A117E1F4FF11B30F7199D3144CE6DFEAFFEF2E331F296E071FA0DF9982CFEA7D43F2E');
...
begin
  ...
  glv := TGlvTypeBParameters.Create(TBigInteger.Create('bb85691939b869c1d087f601554b96b80cb4f55b35f433c2', 16), TBigInteger.Create('3d84f26c12238d7b4f3d516613c1759033b1a5800175d0b1', 16),

    TScalarSplitParameters.Create(TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('71169be7330b3038edb025f1', 16), TBigInteger.Create('-b3fb3400dec5c4adceb8655c', 16)), TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('12511cfe811d0f4e6bc688b4d', 16),
    TBigInteger.Create('71169be7330b3038edb025f1', 16)), TBigInteger.Create('71169be7330b3038edb025f1d0f9', 16), TBigInteger.Create('b3fb3400dec5c4adceb8655d4c94', 16), 208) as IScalarSplitParameters);

  curve := ConfigureCurveGlv(TFpCurve.Create(p, a, b, n, h) as IFpCurve, glv);
  G := ConfigureBasepoint(curve, '04DB4FF10EC057E9AE26B07D0280B7F4341DA5D1B1EAE06C7D9B2F2F6D9C5628A7844163D015BE86344082AA88D95E2F9D');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '04188DA80EB03090F67CBF20EB43A18800F4FF0AFD82FF101207192B95FFC8DA78631011ED6B24CDD573F977A11E794811');
...
begin
  ...
  glv := TGlvTypeBParameters.Create(TBigInteger.Create('fe0e87005b4e83761908c5131d552a850b3f58b749c37cf5b84d6768', 16), TBigInteger.Create('60dcd2104c4cbc0be6eeefc2bdd610739ec34e317f9b33046c9e4788', 16),
    TScalarSplitParameters.Create(TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('6b8cf07d4ca75c88957d9d670591', 16), TBigInteger.Create('-b8adf1378a6eb73409fa6c9c637d', 16)), TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('1243ae1b4d71613bc9f780a03690e', 16),
    TBigInteger.Create('6b8cf07d4ca75c88957d9d670591', 16)), TBigInteger.Create('6b8cf07d4ca75c88957d9d67059037a4', 16), TBigInteger.Create('b8adf1378a6eb73409fa6c9c637ba7f5', 16), 240) as IScalarSplitParameters);

  curve := ConfigureCurveGlv(TFpCurve.Create(p, a, b, n, h) as IFpCurve, glv);
  G := ConfigureBasepoint(curve, '04A1455B334DF099DF30FC28A169A467E9E47075A90F7E650EB6B7A45C7E089FED7FBA344282CAFBD6F7E319F7C0B0BD59E2CA4BDB556D61A5');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '04B70E0CBD6BB4BF7F321390B94A03C1D356C21122343280D6115C1D21BD376388B5F723FB4C22DFE6CD4375A05A07476444D5819985007E34');
...
begin
  ...
  glv := TGlvTypeBParameters.Create(TBigInteger.Create('7ae96a2b657c07106e64479eac3434e99cf0497512f58995c1396c28719501ee', 16), TBigInteger.Create('5363ad4cc05c30e0a5261c028812645a122e22ea20816678df02967c1b23bd72', 16),

    TScalarSplitParameters.Create(TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('3086d221a7d46bcde86c90e49284eb15', 16), TBigInteger.Create('-e4437ed6010e88286f547fa90abfe4c3', 16)),
    TCryptoLibGenericArray<TBigInteger>.Create(TBigInteger.Create('114ca50f7a8e2f3f657c1108d9d44cfd8', 16), TBigInteger.Create('3086d221a7d46bcde86c90e49284eb15', 16)), TBigInteger.Create('3086d221a7d46bcde86c90e49284eb153dab', 16), TBigInteger.Create('e4437ed6010e88286f547fa90abfe4c42212', 16),
    272) as IScalarSplitParameters);

  curve := ConfigureCurveGlv(TFpCurve.Create(p, a, b, n, h) as IFpCurve, glv);
  G := ConfigureBasepoint(curve, '0479BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199C47D08FFB10D4B8');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '046B17D1F2E12C4247F8BCE6E563A440F277037D812DEB33A0F4A13945D898C2964FE342E2FE1A7F9B8EE7EB4A7C0F9E162BCE33576B315ECECBB6406837BF51F5');
...
begin
  ...
  curve := ConfigureCurve(TFpCurve.Create(p, a, b, n, h) as IFpCurve);
  G := ConfigureBasepoint(curve, '04' + 'AA87CA22BE8B05378EB1C71EF320AD746E1D3B628BA79B9859F741E082542A385502F25DBF55296C3A545E3872760AB7' + '3617DE4A96262C6F5D9E98BF9292DC29F8F41DBD289A147CE9DA3113B5F0B8C00A60B1CE1D7E819D7A431D7C90EA0E5F');
...
begin
  ...
  G := ConfigureBasepoint(curve, '04009D73616F35F4AB1407D73562C10F00A52830277958EE84D1315ED31886');
...
begin
  ...
  curve := ConfigureCurve(TF2mCurve.Create(M, Fk, a, b, n, h) as IF2mCurve);
  G := ConfigureBasepoint(curve, '