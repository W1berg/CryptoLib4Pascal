...
type
  ...
  strict private
    ...
    class function Calculate(const AHashInstance: IHash; const ANamePrefix: string = ''; ASize: Int32 = 64 * 1024): string;
...
class function TPerformanceBenchmark.Calculate(const AHashInstance: IHash; const ANamePrefix: string; ASize: Int32): string;
const
...
begin
  ...
  begin
    ...
    MaxRate := Math.Max(Total / (TotalMilliSeconds div 1000) / 1024 / 1024, MaxRate);
  ...
  Result := Format('%s Throughput: %.2f MB/s with Blocks of %s', [Copy(NewName, 2, System.Length(NewName) - 1), MaxRate, BlockSizeAndUnit]);
end;
...
begin
  ...
  AStringList.Append(Calculate(THashFactory.TChecksum.TCRC.CreateCRC(TCRCStandard.CRC32), 'PKZIP_Generic'));

  AStringList.Append(Calculate(THashFactory.TChecksum.TCRC.CreateCRC32_PKZIP, 'Fast'));
  ...
  AStringList.Append(Calculate(THashFactory.THash128.CreateMurmurHash3_x86_128));

  AStringList.Append(Calculate(THashFactory.THash128.CreateMurmurHash3_x64_128));
