...
type
  ...
  var
    ...
    procedure TransformBlock(AData: PByte; ADataLength: Int32; AIndex: Int32); virtual; abstract;
...
procedure TBlockHash.TransformBytes(const AData: THashLibByteArray; AIndex, ALength: Int32);
var
  LPtrData: PByte;
begin
{$IFDEF DEBUG}
...
{$ENDIF DEBUG}
  ...
  begin
    if (FBuffer.Feed(LPtrData, System.Length(AData), AIndex, ALength, FProcessedBytesCount)) then
    begin
      TransformBuffer();
    end;
  end;
  ...
  if (ALength > 0) then
  begin
    FBuffer.Feed(LPtrData, System.Length(AData), AIndex, ALength, FProcessedBytesCount);
  end;
end;
