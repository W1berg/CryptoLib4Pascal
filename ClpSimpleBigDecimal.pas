...
resourcestring
  ...
  SUnEqualScale = 'Only SimpleBigDecimal of Same Scale Allowed in Arithmetic Operations';
...
type
  ...
  public
    ...
    // */
    class function GetInstance(const val: TBigInteger; scale: Int32): TSimpleBigDecimal; static; inline;
...
begin
  ...
  Result := TSimpleBigDecimal.Create(FbigInt.ShiftLeft(newScale - Fscale), newScale);
end;
...
function TSimpleBigDecimal.Divide(const b: TSimpleBigDecimal): TSimpleBigDecimal;
var
...
class function TSimpleBigDecimal.GetInstance(const val: TBigInteger; scale: Int32): TSimpleBigDecimal;
begin
...
function TSimpleBigDecimal.Multiply(const b: TSimpleBigDecimal): TSimpleBigDecimal;
begin
  CheckScale(b);
  Result := TSimpleBigDecimal.Create(FbigInt.Multiply(b.FbigInt), Fscale + Fscale);
end;
{$ENDIF}
...
function TSimpleBigDecimal.Subtract(const b: TSimpleBigDecimal): TSimpleBigDecimal;
begin
  Result := Add(b.Negate());
end;
...
begin
  Result := TSimpleBigDecimal.Create(FbigInt.Subtract(b.ShiftLeft(Fscale)), Fscale);
end;
...
begin
  ...
  if ((floorBigInt.SignValue = -1) and (not(fract.Equals(TBigInteger.Zero)))) then
  begin
  ...
  System.SetString(rightOfPoint, PChar(@fractCharArr[0]), System.Length(fractCharArr));
