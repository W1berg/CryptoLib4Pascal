...
type
  ...
  public
    ...
    class function ValidatePublicPoint(const c: IECCurve; const q: IECPoint): IECPoint; overload; static;
...
class function TECDomainParameters.ValidatePublicPoint(const c: IECCurve; const q: IECPoint): IECPoint;
begin
  if (q = nil) then
...
constructor TECDomainParameters.Create(const curve: IECCurve; const g: IECPoint; const n: TBigInteger);
begin
  Create(curve, g, n, TBigInteger.One, nil);
end;

constructor TECDomainParameters.Create(const curve: IECCurve; const g: IECPoint; const n, h: TBigInteger);
begin
  Create(curve, g, n, h, nil);
end;

constructor TECDomainParameters.Create(const curve: IECCurve; const g: IECPoint; const n, h: TBigInteger; const seed: TCryptoLibByteArray);
begin
  if (curve = nil) then
  ...
  if (not n.IsInitialized) then
    raise EArgumentNilCryptoLibException.CreateResFmt(@SBigIntegerNotInitialized, ['n']);
...
function TECDomainParameters.ValidatePrivateScalar(const d: TBigInteger): TBigInteger;
begin
...
begin
  ...
  result := curve.Equals(other.curve) and g.Equals(other.g) and n.Equals(other.n);
