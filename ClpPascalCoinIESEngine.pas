...
type
  ...
  strict protected
    ...
    function EncryptBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray; override;

    function DecryptBlock(const in_enc: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray; override;

  public

    function ProcessBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray; override;
...
function TPascalCoinIESEngine.DecryptBlock(const in_enc: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  if (Fcipher = nil) then
  begin
    raise EArgumentNilCryptoLibException.CreateRes(@SCipherCannotbeNilInThisMode);
  end
  else
  begin
    // Block cipher mode.
  ...
  Fmac.BlockUpdate(in_enc, inOff + System.Length(FV) + System.Length(T2), inLen - System.Length(FV) - System.Length(T2));
  ...
  Result := Fcipher.DoFinal(in_enc, inOff + System.Length(FV) + Fmac.GetMacSize, inLen - System.Length(FV) - System.Length(T2));
  Exit;
end;

function TPascalCoinIESEngine.EncryptBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  begin
    ...
    if (FIV <> nil) then
    begin
      Fcipher.Init(true, TParametersWithIV.Create(TKeyParameter.Create(K1) as IKeyParameter, FIV));
    end
    else
    begin
  ...
  begin
    MessageToEncryptPadSize := CipherBlockSize - (MessageToEncryptSize mod CipherBlockSize);
  end;
  ...
  System.SetLength(Result, SECURE_HEAD_SIZE + System.Length(FV) + System.Length(T) + System.Length(C));

  PByte(Result)^ := Byte(System.Length(FV));
  (PByte(Result) + 1)^ := Byte(System.Length(T));
  (PWord(Result) + 1)^ := UInt16(MessageToEncryptSize);
  (PWord(Result) + 2)^ := UInt16(MessageToEncryptSize + MessageToEncryptPadSize);

  System.Move(FV[0], Result[SECURE_HEAD_SIZE], System.Length(FV) * System.SizeOf(Byte));

  System.Move(T[0], Result[SECURE_HEAD_SIZE + System.Length(FV)], System.Length(T) * System.SizeOf(Byte));

  System.Move(C[0], Result[SECURE_HEAD_SIZE + System.Length(FV) + System.Length(T)], System.Length(C) * System.SizeOf(Byte));

end;

function TPascalCoinIESEngine.ProcessBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        FV := TArrayUtils.CopyOfRange(&in, inOff + SECURE_HEAD_SIZE, inOff + encLength);
  ...
  try
    ...
    begin
      Result := DecryptBlock(System.Copy(&in, inOff + SECURE_HEAD_SIZE, inLen - SECURE_HEAD_SIZE), inOff, inLen - SECURE_HEAD_SIZE);
      Exit;
    end;
