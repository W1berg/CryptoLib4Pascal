...
type
  ...
  /// ietf specs</see>
  /// </summary>
  TArgon2ParametersGenerator = class sealed(TPbeParametersGenerator, IArgon2ParametersGenerator)
  ...
  public
    ...
    procedure Init(argon2Type: TCryptoLibArgon2Type; argon2Version: TCryptoLibArgon2Version; const password, salt, secret, additional: TCryptoLibByteArray; iterations, memory, parallelism: Int32; memoryCostType: TCryptoLibArgon2MemoryCostType);
    ...
    /// a parameters object representing a key.
    /// </returns>
    function GenerateDerivedParameters(const algorithm: string; keySize: Int32): ICipherParameters; overload; override;
    ...
    /// </returns>
    function GenerateDerivedParameters(const algorithm: string; keySize, ivSize: Int32): ICipherParameters; overload; override;
    ...
    /// a parameters object representing a key.
    /// </returns>
    function GenerateDerivedMacParameters(keySize: Int32): ICipherParameters; override;
...
function TArgon2ParametersGenerator.GenerateDerivedKey(dkLen: Int32): TCryptoLibByteArray;
begin
  result := FPBKDF_Argon2.GetBytes(dkLen);
end;

function TArgon2ParametersGenerator.GenerateDerivedMacParameters(keySize: Int32): ICipherParameters;
var
...
function TArgon2ParametersGenerator.GenerateDerivedParameters(const algorithm: string; keySize: Int32): ICipherParameters;
var
  dKey: TCryptoLibByteArray;
begin
  keySize := keySize div 8;
...
function TArgon2ParametersGenerator.GenerateDerivedParameters(const algorithm: string; keySize, ivSize: Int32): ICipherParameters;
var
  dKey: TCryptoLibByteArray;
  key: IKeyParameter;
begin
...
procedure TArgon2ParametersGenerator.Init(argon2Type: TCryptoLibArgon2Type; argon2Version: TCryptoLibArgon2Version; const password, salt, secret, additional: TCryptoLibByteArray; iterations, memory, parallelism: Int32; memoryCostType: TCryptoLibArgon2MemoryCostType);
var
...
begin
  ...
  case memoryCostType of
    TCryptoLibArgon2MemoryCostType.MemoryAsKB:
      begin
        LArgon2ParametersBuilder.WithVersion(LArgon2Version).WithSalt(salt).WithSecret(secret).WithAdditional(additional).WithIterations(iterations).WithMemoryAsKB(memory).WithParallelism(parallelism);
      end;

    TCryptoLibArgon2MemoryCostType.MemoryPowOfTwo:
      begin
        LArgon2ParametersBuilder.WithVersion(LArgon2Version).WithSalt(salt).WithSecret(secret).WithAdditional(additional).WithIterations(iterations).WithMemoryPowOfTwo(memory).WithParallelism(parallelism);
      end
  else
    begin
      raise EArgumentCryptoLibException.CreateRes(@SArgon2MemoryCostTypeInvalid);
    end;
  end;

  FArgon2Parameters := LArgon2ParametersBuilder.Build();
  LArgon2ParametersBuilder.Clear();
  FPBKDF_Argon2 := TKDF.TPBKDF_Argon2.CreatePBKDF_Argon2(FPassword, FArgon2Parameters);
end;
