...
type
  ...
  var
    ...
    /// instead. <br />
    /// </summary>
    function Blake2BPCreateLeafParam(const ABlake2BConfig: IBlake2BConfig; const ABlake2BTreeConfig: IBlake2BTreeConfig): TBlake2B;
    ...
    function DeepCloneBlake2BInstances(const ALeafHashes: THashLibGenericArray<TBlake2B>): THashLibGenericArray<TBlake2B>;
...
{$IFDEF USE_PASMP}
    procedure PasMPParallelComputationWrapper(const AJob: PPasMPJob; const AThreadIndex: LongInt; const ADataContainer: Pointer; const AFromIndex, AToIndex: TPasMPNativeInt); inline;
{$ENDIF USE_PASMP}
{$IFDEF USE_MTPROCS}
    procedure MTProcsParallelComputationWrapper(AIdx: PtrInt; ADataContainer: Pointer; AItem: TMultiThreadProcItem); inline;
{$ENDIF USE_MTPROCS}
  strict protected
...
function TBlake2BP.Blake2BPCreateLeafParam(const ABlake2BConfig: IBlake2BConfig; const ABlake2BTreeConfig: IBlake2BTreeConfig): TBlake2B;
begin
...
function TBlake2BP.DeepCloneBlake2BInstances(const ALeafHashes: THashLibGenericArray<TBlake2B>): THashLibGenericArray<TBlake2B>;
var
  LIdx: Int32;
begin
...
procedure TBlake2BP.ParallelComputation(AIdx: Int32; ADataContainer: PDataContainer);
var
...
procedure TBlake2BP.PasMPParallelComputationWrapper(const AJob: PPasMPJob; const AThreadIndex: LongInt; const ADataContainer: Pointer; const AFromIndex, AToIndex: TPasMPNativeInt);
begin
...
procedure TBlake2BP.MTProcsParallelComputationWrapper(AIdx: PtrInt; ADataContainer: Pointer; AItem: TMultiThreadProcItem);
begin
...
{$IF DEFINED(USE_PASMP)}
  ...
  GlobalPasMP.Invoke(GlobalPasMP.ParallelFor(ADataContainer, 0, ParallelismDegree - 1, PasMPParallelComputationWrapper));
{$ELSEIF DEFINED(USE_MTPROCS)}
  ProcThreadPool.DoParallel(MTProcsParallelComputationWrapper, 0, ParallelismDegree - 1, ADataContainer);
{$ELSE}
...
procedure TBlake2BP.TransformBytes(const AData: THashLibByteArray; AIndex, ADataLength: Int32);
var
...
begin
  ...
  begin
    ...
    for LIdx := 0 to System.Pred(ParallelismDegree) do
    begin
      LLeafHashes[LIdx].TransformBytes(FBuffer, LIdx * BlockSizeInBytes, BlockSizeInBytes);
    end;
  ...
  System.Inc(LPtrData, LDataLength - (LDataLength mod UInt64(ParallelismDegree * BlockSizeInBytes)));
...
begin
  ...
  begin
    ...
    begin
      ...
      LLeafHashes[LIdx].TransformBytes(FBuffer, LIdx * BlockSizeInBytes, Int32(LLeft));
    end;
