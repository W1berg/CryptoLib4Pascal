...
type
  ...
  class function ExpandToBitLength(bitLength: Int32; const v: TCryptoLibByteArray): TCryptoLibByteArray; static;
  ...
  type
    ...
    public
      ...
      constructor Create(const data: TCryptoLibByteArray); overload;
      constructor Create(bitLength: Int32; const data: TCryptoLibByteArray); overload;
  ...
  procedure NextBytes(const buf: TCryptoLibByteArray; off, len: Int32); overload; override;

  property IsExhausted: Boolean read GetIsExhausted;

  constructor Create(const sources: TCryptoLibGenericArray<ISource>); overload;

  class function From(const values: TCryptoLibMatrixByteArray): IFixedSecureRandom; static;
...
constructor TFixedSecureRandom.Create(const sources: TCryptoLibGenericArray<ISource>);
var
  bOut: TMemoryStream;
...
begin
  ...
  try
    ...
    begin
      raise EInvalidOperationCryptoLibException.CreateRes(@SUnRecognizedImplementation);
    end;
...
class function TFixedSecureRandom.ExpandToBitLength(bitLength: Int32; const v: TCryptoLibByteArray): TCryptoLibByteArray;
var
...
begin
  ...
  begin
    ...
    System.Move(lv[0], tmp[System.Length(tmp) - System.Length(lv)], System.Length(lv));
...
class function TFixedSecureRandom.From(const values: TCryptoLibMatrixByteArray): IFixedSecureRandom;
var
  bOut: TMemoryStream;
  i: Int32;
...
begin
  System.Move(F_data[F_index], buf[0], System.Length(buf) * System.SizeOf(Byte));

  F_index := F_index + System.Length(buf);
end;

procedure TFixedSecureRandom.NextBytes(const buf: TCryptoLibByteArray; off, len: Int32);
begin
...
procedure TFixedSecureRandom.TRandomChecker.NextBytes(const bytes: TCryptoLibByteArray);
begin

  System.Move(Fdata[Findex], bytes[0], System.Length(bytes) * System.SizeOf(Byte));
...
constructor TFixedSecureRandom.TBigIntegerSource.Create(bitLength: Int32; const data: TCryptoLibByteArray);
begin
  inherited Create(ExpandToBitLength(bitLength, data));
end;

constructor TFixedSecureRandom.TBigIntegerSource.Create(const data: TCryptoLibByteArray);
begin
  inherited Create(data);
end;

constructor TFixedSecureRandom.TBigIntegerSource.Create(bitLength: Int32; const hexData: string);
begin
