...
type
  ...
  private
    ...
    procedure DoIESCipher_Encryption_Decryption_TestWithIV(const KeyPair: IAsymmetricCipherKeyPair; const param: IAlgorithmParameterSpec; const Random: ISecureRandom; const PlainText: string);
...
procedure TTestIESCipher.DoIESCipher_Encryption_Decryption_TestWithIV(const KeyPair: IAsymmetricCipherKeyPair; const param: IAlgorithmParameterSpec; const Random: ISecureRandom; const PlainText: string);
var
...
begin
  ...
  CipherEncrypt.Init(True, KeyPair.Public as IECPublicKeyParameters, param, Random);
  CipherTextBytes := CipherEncrypt.DoFinal(PlainTextBytes);

  // Decryption
  CipherDecrypt := TIESCipher.Create(GetECIESAES256CBCEngine);
  CipherDecrypt.Init(False, KeyPair.Private as IECPrivateKeyParameters, param, Random);
  DecryptionResultBytes := CipherDecrypt.DoFinal(CipherTextBytes);

  if (not AreEqual(PlainTextBytes, DecryptionResultBytes)) then
  begin
    Fail(Format('Decryption Failed - Expected %s but got %s', [EncodeHex(PlainTextBytes), EncodeHex(DecryptionResultBytes)]));
  end;
end;
...
begin
  ...
  KDFInstance := TKdf2BytesGenerator.Create(TDigestUtilities.GetDigest('SHA-256'));
  ...
  Cipher := TPaddedBufferedBlockCipher.Create(blockCipher, TPkcs7Padding.Create() as IPkcs7Padding); // Pkcs7Padding

  // Cipher := TPaddedBufferedBlockCipher.Create(blockCipher,
  // TZeroBytePadding.Create() as IZeroBytePadding); // ZeroBytePadding

  result := TIESEngine.Create(ECDHBasicAgreementInstance, KDFInstance, DigestMACInstance, Cipher);
end;
...
begin
  ...
  ecSpec := TECDomainParameters.Create(Lcurve.Curve, Lcurve.G, Lcurve.N, Lcurve.H, Lcurve.GetSeed);
  RandomInstance := TSecureRandom.Create();
  KeyPairGeneratorInstance.Init(TECKeyGenerationParameters.Create(ecSpec, RandomInstance));
...
procedure TTestIESCipher.TestIESCipher_Random_Values_Encryption_Decryption_AES256_CBC_PKCS7PADDING;
var
  RandomInstance: ISecureRandom;
  PlainText: string;
  I: Int32;
...
begin
  ...
  begin
    ...
    DoIESCipher_Encryption_Decryption_TestWithIV(GetECKeyPair, GetIESParameterSpec, RandomInstance, PlainText);
