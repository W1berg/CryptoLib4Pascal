...
type
  ...
  strict private
    ...
    class function HaveChar(c: Char; const list: TSimpleBaseLibCharArray): Boolean; static; inline;

  public

    class function TrimRight(const S: string; const trimchars: TSimpleBaseLibCharArray): string; static; inline;

    class function AreArraysEqual(const A, B: TSimpleBaseLibByteArray): Boolean; static;
    ...
    class function StringToCharArray(const S: string): TSimpleBaseLibCharArray; static; inline;

  end;

implementation

class function TUtilities.AreArraysEqual(const A, B: TSimpleBaseLibByteArray): Boolean;
begin
...
class function TUtilities.HaveChar(c: Char; const list: TSimpleBaseLibCharArray): Boolean;
var
  I: Int32;
begin
  I := 0;
...
begin
  Result := (ch = ' ') or (Ord(ch) = $85) or (Ord(ch) = $A0) or ((Ord(ch) >= $09) and (Ord(ch) <= $0D));
end;
...
class function TUtilities.StringToCharArray(const S: string): TSimpleBaseLibCharArray;
begin
  Result := nil;
...
class function TUtilities.TrimRight(const S: string; const trimchars: TSimpleBaseLibCharArray): string;
var
