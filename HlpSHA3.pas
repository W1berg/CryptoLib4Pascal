...
resourcestring
  SInvalidXOFSize = 'XOFSize in Bits must be Multiples of 8 and be Greater than Zero Bytes';
...
{$REGION 'Consts'}
  ...
  const
    ...
    RC: array [0 .. 23] of UInt64 = (UInt64($0000000000000001), UInt64($0000000000008082), UInt64($800000000000808A), UInt64($8000000080008000), UInt64($000000000000808B), UInt64($0000000080000001), UInt64($8000000080008081), UInt64($8000000000008009), UInt64($000000000000008A),
      UInt64($0000000000000088), UInt64($0000000080008009), UInt64($000000008000000A), UInt64($000000008000808B), UInt64($800000000000008B), UInt64($8000000000008089), UInt64($8000000000008003), UInt64($8000000000008002), UInt64($8000000000000080), UInt64($000000000000800A),
      UInt64($800000008000000A), UInt64($8000000080008081), UInt64($8000000000008080), UInt64($0000000080000001), UInt64($8000000080008008));
...
type
  ...
  public
    procedure Initialize(); override;
    function GetResult(): THashLibByteArray; override;
    procedure TransformBytes(const AData: THashLibByteArray; AIndex, ADataLength: Int32); override;
    function TransformFinal(): IHashResult; override;
    procedure DoOutput(const ADestination: THashLibByteArray; ADestinationOffset, AOutputLength: UInt64);
  end;
...
type
  ...
  public
    ...
    class function BytePad(const AInput: THashLibByteArray; AW: Int32): THashLibByteArray; static;
    class function EncodeString(const AInput: THashLibByteArray): THashLibByteArray; static;
  end;
...
type
  TKMACNotBuildInAdapter = class abstract(THash, IKMAC, IKMACNotBuildIn, ICrypto, ICryptoNotBuildIn)
...
type
  ...
  strict private
    constructor Create(const AKMACKey, ACustomization: THashLibByteArray; AOutputLengthInBits: UInt64); overload;
    constructor Create(const AHash: IHash; const AKMACKey: THashLibByteArray; AOutputLengthInBits: UInt64); overload;
...
type
  ...
  strict private
    ...
    constructor Create(const AKMACKey, ACustomization: THashLibByteArray); overload;
    constructor Create(const AHash: IHash; const AKMACKey: THashLibByteArray); overload;
  ...
  public
    function Clone(): IHash; override;
    class function CreateKMAC128XOF(const AKMACKey, ACustomization: THashLibByteArray; AXofSizeInBits: UInt64): IKMAC; static;
...
type
  ...
  public
    function Clone(): IHash; override;
    class function CreateKMAC256XOF(const AKMACKey, ACustomization: THashLibByteArray; AXofSizeInBits: UInt64): IKMAC; static;
...
begin
  ...
  TConverters.le64_copy(PUInt64(FState), 0, PByte(Result), 0, System.Length(Result));
end;
...
{$IFDEF USE_UNROLLED_VARIANT}
  Aba, Abe, Abi, Abo, Abu, Aga, Age, Agi, Ago, Agu, Aka, Ake, Aki, Ako, Aku, Ama, Ame, Ami, Amo, Amu, Asa, Ase, Asi, Aso, Asu, BCa, BCe, BCi, BCo, BCu, Eba, Ebe, Ebi, Ebo, Ebu, Ega, Ege, Egi, Ego, Egu, Eka, Eke, Eki, Eko, Eku, Ema, Eme, Emi, Emo, Emu, Esa, Ese, Esi, Eso, Esu: UInt64;
  LRound: Int32;
{$ELSE}
...
{$ELSE}
  for j := 0 to 23 do
  begin
    Ca := FState[00] xor FState[05] xor FState[10] xor FState[15] xor FState[20];
    Ce := FState[01] xor FState[06] xor FState[11] xor FState[16] xor FState[21];
    Ci := FState[02] xor FState[07] xor FState[12] xor FState[17] xor FState[22];
    Co := FState[03] xor FState[08] xor FState[13] xor FState[18] xor FState[23];
    Cu := FState[04] xor FState[09] xor FState[14] xor FState[19] xor FState[24];
...
procedure TShake.DoOutput(const ADestination: THashLibByteArray; ADestinationOffset, AOutputLength: UInt64);
var
  LDiff, LCount, LBlockOffset: UInt64;
begin

  if (UInt64(System.Length(ADestination)) - ADestinationOffset) < AOutputLength then
  begin
  ...
  begin
    ...
    System.Move(FShakeBuffer[LBlockOffset], ADestination[ADestinationOffset], LCount);
...
procedure TShake.TransformBytes(const AData: THashLibByteArray; AIndex, ADataLength: Int32);
begin
  if FFinalized then
  begin
    raise EInvalidOperationHashLibException.CreateResFmt(@SWritetoXofAfterReadError, [name]);
  end;
...
class function TCShake.BytePad(const AInput: THashLibByteArray; AW: Int32): THashLibByteArray;
var
  LBuffer: THashLibByteArray;
  LPadLength: Int32;
begin
...
class function TCShake.EncodeString(const AInput: THashLibByteArray): THashLibByteArray;
begin
  ...
  Result := TArrayUtils.Concatenate(LeftEncode(UInt64(System.Length(AInput) * 8)), AInput);
end;
...
procedure TKMACNotBuildInAdapter.DoOutput(const ADestination: THashLibByteArray; ADestinationOffset, AOutputLength: UInt64);
begin
...
procedure TKMACNotBuildInAdapter.TransformBytes(const AData: THashLibByteArray; AIndex, ALength: Int32);
begin
...
begin
  LBuffer := GetResult();
{$IFDEF DEBUG}
  System.Assert(UInt64(System.Length(LBuffer)) = ((FHash as IXOF).XOFSizeInBits shr 3));
{$ENDIF DEBUG}
  Initialize();
  Result := THashResult.Create(LBuffer);
end;
...
begin
  LHashInstance := TKMAC128.Create(FHash.Clone(), FKey, (FHash as IXOF).XOFSizeInBits);
  Result := LHashInstance as IHash;
  Result.BufferSize := BufferSize;
end;

constructor TKMAC128.Create(const AKMACKey, ACustomization: THashLibByteArray; AOutputLengthInBits: UInt64);
begin
  Create(TCShake_128.Create(TConverters.ConvertStringToBytes('KMAC', TEncoding.UTF8), ACustomization) as IHash, AKMACKey, AOutputLengthInBits);
end;

constructor TKMAC128.Create(const AHash: IHash; const AKMACKey: THashLibByteArray; AOutputLengthInBits: UInt64);
begin
...
class function TKMAC128.CreateKMAC128(const AKMACKey, ACustomization: THashLibByteArray; AOutputLengthInBits: UInt64): IKMAC;
begin
  Result := TKMAC128.Create(AKMACKey, ACustomization, AOutputLengthInBits) as IKMAC;
end;
...
constructor TKMAC128XOF.Create(const AHash: IHash; const AKMACKey: THashLibByteArray);
begin
  inherited Create(Int32(THashSize.hsHashSize128));
  SetKey(AKMACKey);
  FHash := AHash;
...
constructor TKMAC128XOF.Create(const AKMACKey, ACustomization: THashLibByteArray);
begin
  Create(TCShake_128.Create(TConverters.ConvertStringToBytes('KMAC', TEncoding.UTF8), ACustomization) as IHash, AKMACKey);
end;

class function TKMAC128XOF.CreateKMAC128XOF(const AKMACKey, ACustomization: THashLibByteArray; AXofSizeInBits: UInt64): IKMAC;
var
  LXof: IXOF;
begin
...
begin
  LHashInstance := TKMAC256.Create(FHash.Clone(), FKey, (FHash as IXOF).XOFSizeInBits);
  Result := LHashInstance as IHash;
  Result.BufferSize := BufferSize;
end;

constructor TKMAC256.Create(const AKMACKey, ACustomization: THashLibByteArray; AOutputLengthInBits: UInt64);
begin
  Create(TCShake_256.Create(TConverters.ConvertStringToBytes('KMAC', TEncoding.UTF8), ACustomization) as IHash, AKMACKey, AOutputLengthInBits);
end;

constructor TKMAC256.Create(const AHash: IHash; const AKMACKey: THashLibByteArray; AOutputLengthInBits: UInt64);
begin
...
class function TKMAC256.CreateKMAC256(const AKMACKey, ACustomization: THashLibByteArray; AOutputLengthInBits: UInt64): IKMAC;
begin
  Result := TKMAC256.Create(AKMACKey, ACustomization, AOutputLengthInBits) as IKMAC;
end;
...
constructor TKMAC256XOF.Create(const AHash: IHash; const AKMACKey: THashLibByteArray);
begin
  inherited Create(Int32(THashSize.hsHashSize256));
  SetKey(AKMACKey);
  FHash := AHash;
...
constructor TKMAC256XOF.Create(const AKMACKey, ACustomization: THashLibByteArray);
begin
  Create(TCShake_256.Create(TConverters.ConvertStringToBytes('KMAC', TEncoding.UTF8), ACustomization) as IHash, AKMACKey);
end;

class function TKMAC256XOF.CreateKMAC256XOF(const AKMACKey, ACustomization: THashLibByteArray; AXofSizeInBits: UInt64): IKMAC;
var
  LXof: IXOF;
begin
