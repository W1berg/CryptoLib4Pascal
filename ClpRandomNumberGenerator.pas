...
resourcestring
  ...
  SRandomNumberGeneratorOutputBufferNil = 'Random Number Generator Output Buffer Cannot Be Nil';

type
  TRandomNumberGenerator = class abstract(TInterfacedObject, IRandomNumberGenerator)

  strict protected
    class procedure ValidateOutputBufferNotNull(const ABuffer: TCryptoLibByteArray); static; inline;
...
{$SCOPEDENUMS OFF}
  ...
  class function CreateRNG(rngMode: TRandomNumberGeneratorMode): IRandomNumberGenerator; overload; static;
...
type
  TOSRandomNumberGenerator = class sealed(TRandomNumberGenerator, IOSRandomNumberGenerator)
...
type
  TAESPRNGRandomNumberGenerator = class sealed(TRandomNumberGenerator, IAESPRNGRandomNumberGenerator)
...
class procedure TRandomNumberGenerator.ValidateOutputBufferNotNull(const ABuffer: TCryptoLibByteArray);
begin
  if ABuffer = nil then
  begin
    raise EArgumentNilCryptoLibException.CreateRes(@SRandomNumberGeneratorOutputBufferNil);
  end;
end;
...
class function TRandomNumberGenerator.CreateRNG(rngMode: TRandomNumberGeneratorMode): IRandomNumberGenerator;
begin
...
procedure TAESPRNGRandomNumberGenerator.GetBytes(const data: TCryptoLibByteArray);
begin
  ValidateOutputBufferNotNull(data);
  TAESPRNGRandom.GetBytes(data);
end;

procedure TAESPRNGRandomNumberGenerator.GetNonZeroBytes(const data: TCryptoLibByteArray);
begin
  ValidateOutputBufferNotNull(data);
