...
type
  ...
  var
    FZERO_SEED, FTEST_SEED, Fexpected0SHA1, FnoCycle0SHA1, Fexpected0SHA256, FnoCycle0SHA256, Fexpected100SHA1, Fexpected100SHA256, FexpectedTestSHA1, FexpectedTestSHA256, Fsha1Xors, Fsha256Xors: TBytes;

    procedure doExpectedTest(const digest: IDigest; seed: Int32; const expected: TBytes); overload;
    procedure doExpectedTest(const digest: IDigest; const seed, expected: TBytes); overload;
    procedure doExpectedTest(const digest: IDigest; seed: Int32; const expected, noCycle: TBytes); overload;
    procedure doCountTest(const digest: IDigest; const seed, expectedXors: TBytes);
...
procedure TTestDigestRandomNumber.doCountTest(const digest: IDigest; const seed, expectedXors: TBytes);
var
...
procedure TTestDigestRandomNumber.doExpectedTest(const digest: IDigest; seed: Int32; const expected: TBytes);
begin
  doExpectedTest(digest, seed, expected, nil);
end;

procedure TTestDigestRandomNumber.doExpectedTest(const digest: IDigest; seed: Int32; const expected, noCycle: TBytes);
var
  rGen: IDigestRandomGenerator;
  output: TBytes;
  i: Int32;
...
procedure TTestDigestRandomNumber.doExpectedTest(const digest: IDigest; const seed, expected: TBytes);
var
  rGen: IDigestRandomGenerator;
  output: TBytes;
  i: Int32;
...
begin
  ...
  Fexpected0SHA256 := DecodeHex('587e2dfd597d086e47ddcd343eac983a5c913bef8c6a1a560a5c1bc3a74b0991');
  FnoCycle0SHA256 := DecodeHex('e5776c4483486ba7be081f4e1b9dafbab25c8fae290fd5474c1ceda2c16f9509');
  Fexpected100SHA1 := DecodeHex('b9d924092546e0876cafd4937d7364ebf9efa4be');
  Fexpected100SHA256 := DecodeHex('fbc4aa54b948b99de104c44563a552899d718bb75d1941cc62a2444b0506abaf');
  FexpectedTestSHA1 := DecodeHex('e9ecef9f5306daf1ac51a89a211a64cb24415649');
  FexpectedTestSHA256 := DecodeHex('bdab3ca831b472a2fa09bd1bade541ef16c96640a91fcec553679a136061de98');

  Fsha1Xors := DecodeHex('7edcc1216934f3891b03ffa65821611a3e2b1f79');
  Fsha256Xors := DecodeHex('5ec48189cc0aa71e79c707bc3c33ffd47bbba368a83d6cfebf3cd3969d7f3eed');
end;
...
begin
  doExpectedTest(TDigestUtilities.GetDigest('SHA-1'), 0, Fexpected0SHA1, FnoCycle0SHA1);
  doExpectedTest(TDigestUtilities.GetDigest('SHA-256'), 0, Fexpected0SHA256, FnoCycle0SHA256);

  doExpectedTest(TDigestUtilities.GetDigest('SHA-1'), 100, Fexpected100SHA1);
  doExpectedTest(TDigestUtilities.GetDigest('SHA-256'), 100, Fexpected100SHA256);

  doExpectedTest(TDigestUtilities.GetDigest('SHA-1'), FZERO_SEED, Fexpected0SHA1);
  doExpectedTest(TDigestUtilities.GetDigest('SHA-256'), FZERO_SEED, Fexpected0SHA256);

  doExpectedTest(TDigestUtilities.GetDigest('SHA-1'), FTEST_SEED, FexpectedTestSHA1);
  doExpectedTest(TDigestUtilities.GetDigest('SHA-256'), FTEST_SEED, FexpectedTestSHA256);
