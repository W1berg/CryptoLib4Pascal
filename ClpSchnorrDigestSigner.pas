...
resourcestring
  ...
  SSchnorrDigestSignerNotInitializedForSignatureGeneration = 'SchnorrDigestSigner not Initialized for Signature Generation.';
  SSchnorrDigestSignerNotInitializedForVerification = 'SchnorrDigestSigner not Initialized for Verification';
...
type
  ...
  public
    ...
    // overload;
    constructor Create(const Schnorr: ISchnorrExt; const digest: IDigest; const encoding: ISchnorrEncoding);
    ...
    /// </summary>
    procedure BlockUpdate(const input: TCryptoLibByteArray; inOff, length: Int32); virtual;
...
procedure TSchnorrDigestSigner.BlockUpdate(const input: TCryptoLibByteArray; inOff, length: Int32);
begin
  if input <> nil then
  begin
...
constructor TSchnorrDigestSigner.Create(const Schnorr: ISchnorrExt; const digest: IDigest; const encoding: ISchnorrEncoding);
begin
  inherited Create();
  FSchnorr := Schnorr;
  FDigest := digest;
...
begin
  ...
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SSchnorrDigestSignerNotInitializedForSignatureGeneration);
  end;
...
procedure TSchnorrDigestSigner.Init(forSigning: Boolean; const parameters: ICipherParameters);
var
...
function TSchnorrDigestSigner.VerifySignature(const signature: TCryptoLibByteArray): Boolean;
var
  sig: TCryptoLibGenericArray<TBigInteger>;
begin
  if (FForSigning) then
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SSchnorrDigestSignerNotInitializedForVerification);
  end;
