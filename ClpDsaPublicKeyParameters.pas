...
type
  TDsaPublicKeyParameters = class sealed(TDsaKeyParameters, IDsaPublicKeyParameters)

  strict private
  var
    Fy: TBigInteger;

    class function Validate(const y: TBigInteger; const parameters: IDsaParameters): TBigInteger; static; inline;
...
class function TDsaPublicKeyParameters.Validate(const y: TBigInteger; const parameters: IDsaParameters): TBigInteger;
begin
  ...
  begin
    if ((y.CompareTo(TBigInteger.Two) < 0) or (y.CompareTo(parameters.P.Subtract(TBigInteger.Two)) > 0) or (not(y.ModPow(parameters.Q, parameters.P).Equals(TBigInteger.One)))) then
    begin
...
constructor TDsaPublicKeyParameters.Create(const y: TBigInteger; const parameters: IDsaParameters);
begin
  inherited Create(false, parameters);
  Fy := Validate(y, parameters);
end;

function TDsaPublicKeyParameters.Equals(const other: IDsaPublicKeyParameters): Boolean;
begin
  if other = nil then
  begin
    result := false;
