...
{$REGION 'Consts'}
  ...
  const
    SRot: array [0 .. 19] of Int32 = (5, 11, 7, 15, 6, 13, 8, 14, 7, 12, 9, 11, 8, 15, 6, 12, 9, 14, 5, 13);

    Stor: array [0 .. 19] of Int32 = (27, 21, 25, 17, 26, 19, 24, 18, 25, 20, 23, 21, 24, 17, 26, 20, 23, 18, 27, 19);

    SIndex: array [0 .. 79] of Int32 = (18, 0, 1, 2, 3, 19, 4, 5, 6, 7, 16, 8, 9, 10, 11, 17, 12, 13, 14, 15, 18, 3, 6, 9, 12, 19, 15, 2, 5, 8, 16, 11, 14, 1, 4, 17, 7, 10, 13, 0, 18, 12, 5, 14, 7, 19, 0, 9, 2, 11, 16, 4, 13, 6, 15, 17, 8, 1, 10, 3, 18, 7, 2, 13, 8, 19, 3, 14, 9, 4, 16, 15, 10, 5,
      0, 17, 11, 6, 1, 12);
...
procedure THAS160.TransformBlock(AData: PByte; ADataLength: Int32; AIndex: Int32);
var
  A, B, C, D, E, T: UInt32;
  R: Int32;
...
begin
  ...
  while R < 20 do
  begin
    T := LData[SIndex[R]] + (A shl SRot[R] or A shr Stor[R]) + ((B and C) or (not B and D)) + E;
    E := D;
    D := C;
  ...
  while R < 40 do
  begin
    T := LData[SIndex[R]] + $5A827999 + (A shl SRot[R - 20] or A shr Stor[R - 20]) + (B xor C xor D) + E;
    E := D;
    D := C;
  ...
  while R < 60 do
  begin
    T := LData[SIndex[R]] + $6ED9EBA1 + (A shl SRot[R - 40] or A shr Stor[R - 40]) + (C xor (B or not D)) + E;
    E := D;
    D := C;
  ...
  while R < 80 do
  begin
    T := LData[SIndex[R]] + $8F1BBCDC + (A shl SRot[R - 60] or A shr Stor[R - 60]) + (B xor C xor D) + E;
    E := D;
    D := C;
