...
begin
  ...
  for x := 0 to System.Pred(System.SizeOf(ChunkSizes) div System.SizeOf(Int32)) do
  begin
    size := ChunkSizes[x];
    ...
    begin
      ...
      temp := System.Copy(ChunkedData, (i - size) + 1, System.Length(ChunkedData) - ((i - size)));
      FCRC.TransformString(temp, TEncoding.UTF8);

      ActualString := FCRC.TransformFinal().ToString();

      ExpectedString := THashFactory.TChecksum.TCRC.CreateCRC(Idx).ComputeString(ChunkedData, TEncoding.UTF8).ToString();

      CheckEquals(ExpectedString, ActualString, Format('Expected %s but got %s. %s', [ExpectedString, ActualString, FCRC.Name]));
...
begin
  ...
  begin
    ...
    CheckEquals(ExpectedString, ActualString, Format('Expected %s but got %s. %s', [ExpectedString, ActualString, Original.Name]));
  end;
end;
...
begin
  ...
  begin
    ...
    CheckNotEquals(Original.BufferSize, Copy.BufferSize, Format('Expected %d but got %d. %s', [Original.BufferSize, Copy.BufferSize, Original.Name]));
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ExpectedString := FCRC32Fast.ComputeString(ChunkedData, TEncoding.UTF8).ToString();

      CheckEquals(ExpectedString, ActualString, Format('Expected %s but got %s. %s', [ExpectedString, ActualString, FCRC32Fast.Name]));
