...
type
  ...
  /// </summary>
  TScryptParametersGenerator = class sealed(TPbeParametersGenerator, IScryptParametersGenerator)
  ...
  public
    ...
    procedure Init(const password, salt: TCryptoLibByteArray; cost, blockSize, parallelism: Int32);
...
function TScryptParametersGenerator.GenerateDerivedKey(dkLen: Int32): TCryptoLibByteArray;
begin
  result := FPBKDF_Scrypt.GetBytes(dkLen);
end;

function TScryptParametersGenerator.GenerateDerivedMacParameters(keySize: Int32): ICipherParameters;
var
...
function TScryptParametersGenerator.GenerateDerivedParameters(const algorithm: string; keySize: Int32): ICipherParameters;
var
  dKey: TCryptoLibByteArray;
begin
  keySize := keySize div 8;
...
function TScryptParametersGenerator.GenerateDerivedParameters(const algorithm: string; keySize, ivSize: Int32): ICipherParameters;
var
  dKey: TCryptoLibByteArray;
  key: IKeyParameter;
begin
...
procedure TScryptParametersGenerator.Init(const password, salt: TCryptoLibByteArray; cost, blockSize, parallelism: Int32);
begin
  FPassword := System.Copy(password);
  FSalt := System.Copy(salt);
  FPBKDF_Scrypt := TKDF.TPBKDF_Scrypt.CreatePBKDF_Scrypt(FPassword, FSalt, cost, blockSize, parallelism);
end;
