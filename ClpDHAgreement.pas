...
resourcestring
  SDHPublicKeyWrongParameter = 'Diffie-Hellman Public Key has Wrong Parameters.';
...
type
  ...
  public
    ...
    /// </summary>
    function CalculateAgreement(const pub: IDHPublicKeyParameters; const &message: TBigInteger): TBigInteger;
...
function TDHAgreement.CalculateAgreement(const pub: IDHPublicKeyParameters; const &message: TBigInteger): TBigInteger;
var
  p, peerY: TBigInteger;
begin
  ...
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SMessageNotInitialized);
  end;
  ...
  if ((not peerY.IsInitialized) or (peerY.CompareTo(TBigInteger.One) <= 0) or (peerY.CompareTo(p.Subtract(TBigInteger.One)) >= 0)) then
  begin
...
begin
  ...
  dhGen.Init(TDHKeyGenerationParameters.Create(Frandom, FdhParams) as IDHKeyGenerationParameters);
