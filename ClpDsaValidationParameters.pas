...
type
  ...
  public
    constructor Create(const seed: TCryptoLibByteArray; counter: Int32); overload;
    constructor Create(const seed: TCryptoLibByteArray; counter, usageIndex: Int32); overload;

    function Equals(const other: IDsaValidationParameters): Boolean; reintroduce;
...
constructor TDsaValidationParameters.Create(const seed: TCryptoLibByteArray; counter: Int32);
begin
  Create(seed, counter, -1);
end;

constructor TDsaValidationParameters.Create(const seed: TCryptoLibByteArray; counter, usageIndex: Int32);
begin
  inherited Create();
  if (seed = nil) then
  begin
...
function TDsaValidationParameters.Equals(const other: IDsaValidationParameters): Boolean;
begin
  if other = nil then
  begin
    result := False;
  ...
  result := (counter = other.counter) and TArrayUtils.AreEqual(seed, other.seed);
end;
