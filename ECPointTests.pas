...
type
  ...
  var
    ...
    // */
    procedure ImplTestAdd(const p: TCryptoLibGenericArray<IECPoint>; const infinity: IECPoint);
    ...
    procedure ImplAddSubtractMultiplyTwiceEncodingTest(const curve: IECCurve; const q: IECPoint; const n: TBigInteger);

    procedure ImplSqrtTest(const c: IECCurve);

    procedure ImplValidityTest(const c: IECCurve; const g: IECPoint);

    procedure ImplAddSubtractMultiplyTwiceEncodingTestAllCoords(const x9ECParameters: IX9ECParameters);

    function SolveQuadraticEquation(const c: IECCurve; const rhs: IECFieldElement): IECFieldElement;

    function ConfigureBasepoint(const curve: IECCurve; const encoding: string): IX9ECPoint;
...
procedure TTestECPoint.AssertPointsEqual(const msg: string; const a, b: IECPoint);
begin
  CheckEquals(True, a.Equals(b), msg);
  CheckEquals(True, b.Equals(a), msg);
end;

function TTestECPoint.ConfigureBasepoint(const curve: IECCurve; const encoding: string): IX9ECPoint;
begin
...
procedure TTestECPoint.ImplAddSubtractMultiplyTwiceEncodingTest(const curve: IECCurve; const q: IECPoint; const n: TBigInteger);
var
  infinity, p: IECPoint;
...
procedure TTestECPoint.ImplAddSubtractMultiplyTwiceEncodingTestAllCoords(const x9ECParameters: IX9ECParameters);
var
  n, b: TBigInteger;
  g, sg, q: IECPoint;
  c, sc: IECCurve;
...
begin
  ...
  begin
    ...
    while (count < 10) do
    begin
      nonSquare := TBigIntegers.CreateRandomInRange(TBigInteger.Two, pMinusOne, FRandom);
      if (not nonSquare.ModPow(legendreExponent, p).Equals(TBigInteger.One)) then
      begin
...
procedure TTestECPoint.ImplTestAdd(const p: TCryptoLibGenericArray<IECPoint>; const infinity: IECPoint);
var
  i: Int32;
begin
...
begin
  ...
  repeat
    ...
    AssertPointsEqual('Results of Add() and Multiply() are inconsistent ' + i.ToString, adder, multiplier);
...
procedure TTestECPoint.ImplTestThreeTimes(const p: TCryptoLibGenericArray<IECPoint>);
var
  Lp, _3P: IECPoint;
begin
  Lp := p[0];
...
function TTestECPoint.SolveQuadraticEquation(const c: IECCurve; const rhs: IECFieldElement): IECFieldElement;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      AssertBigIntegersEqual(x9A.curve.a.ToBigInteger(), x9B.curve.a.ToBigInteger());
      AssertBigIntegersEqual(x9A.curve.b.ToBigInteger(), x9B.curve.b.ToBigInteger());
...
begin
  try
    FpInstance.Fcurve.CreatePoint(TBigInteger.ValueOf(12), default (TBigInteger));
  ...
  try
    FpInstance.Fcurve.CreatePoint(default (TBigInteger), TBigInteger.ValueOf(12));
    Fail('expected EArgumentCryptoLibException');
  except
    on e: EArgumentCryptoLibException do
    begin
  ...
  try
    FpInstance.Fcurve.CreatePoint(TBigInteger.Create('1011'), default (TBigInteger));
    Fail('expected EArgumentCryptoLibException');
  except
    on e: EArgumentCryptoLibException do
    begin
  ...
  try
    FpInstance.Fcurve.CreatePoint(default (TBigInteger), TBigInteger.Create('1011'));
    Fail('expected EArgumentCryptoLibException');
  except
    on e: EArgumentCryptoLibException do
    begin
...
begin
  ...
  *)
  p := FromHex('997ABB1F0A563FDA65C61198DAD0657A' + '416C0CE19CB48261BE9AE358B3E01A2E' + 'F40AAB27E2FC0F1B228730D531A59CB0' + 'E791B39FF7C88A19356D27F4A666A6D0' + 'E26C6487326B4CD4512AC5CD65681CE1' + 'B6AFF4A831852A82A7CF3C521C3C09AA' + '9F94D6AF56971F1FFCE3E82389857DB0' +
  ...
  g := ConfigureBasepoint(curve, '04'
    // Px
    + '53FC09EE332C29AD0A7990053ED9B52A' + '2B1A2FD60AEC69C698B2F204B6FF7CBF' + 'B5EDB6C0F6CE2308AB10DB9030B09E10' + '43D5F22CDB9DFA55718BD9E7406CE890' + '9760AF765DD5BCCB337C86548B72F2E1' + 'A702C3397A60DE74A7C1514DBA66910D' + 'D5CFB4CC80728D87EE9163A5B63F73EC' + '80EC46C4967E0979880DC8ABEAE63895'
    // Py
    + '0A8249063F6009F1F9F1F0533634A135' + 'D3E82016029906963D778D821E141178' + 'F5EA69F4654EC2B9E7F7F5E5F0DE55F6' + '6B598CCF9A140B2E416CFF0CA9E032B9' + '70DAE117AD547C6CCAD696B5B7652FE0' + 'AC6F1E80164AA989492D979FC5A4D5F2' + '13515AD7E9CB99A980BDAD5AD5BB4636' +
...
begin
  ...
  begin
    Fp[i] := Fcurve.CreatePoint(TBigInteger.Create(IntToStr(FpointSource[2 * i])), TBigInteger.Create(IntToStr(FpointSource[2 * i + 1])));
  end;
end;
...
begin
  ...
  FpointSource := TCryptoLibStringArray.Create('0010', '1111', '1100', '1100', '0001', '0001', '1011', '0010');
...
begin
  ...
  begin
    Fp[i] := Fcurve.CreatePoint(TBigInteger.Create(FpointSource[2 * i], 2), TBigInteger.Create(FpointSource[(2 * i) + 1], 2));
  end;
end;
