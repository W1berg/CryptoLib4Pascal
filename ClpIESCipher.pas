...
resourcestring
  SInvalidPublicKey = 'Must be Passed Recipient''s Public EC Key for Encryption';
  SInvalidPrivateKey = 'Must be Passed Recipient''s Private EC Key for Decryption';
...
type
  ...
  public
    procedure Init(ForEncryption: Boolean; const Key: ICipherParameters; const EngineSpec: IAlgorithmParameterSpec; const Random: ISecureRandom);

    procedure ProcessBytes(const input: TCryptoLibByteArray); overload;
    procedure ProcessBytes(const input: TCryptoLibByteArray; inputOffset, inputLen: Int32); overload;

    function DoFinal(const input: TCryptoLibByteArray): TCryptoLibByteArray; overload;

    function DoFinal(const input: TCryptoLibByteArray; inputOffset, inputLen: Int32): TCryptoLibByteArray; overload;

    function DoFinal(const input: TCryptoLibByteArray; inputOffset, inputLen: Int32; const output: TCryptoLibByteArray; outputOffset: Int32): Int32; overload;
...
function TIESCipher.DoFinal(const input: TCryptoLibByteArray; inputOffset, inputLen: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  params := TIESWithCipherParameters.Create(FEngineSpec.GetDerivationV, FEngineSpec.GetEncodingV, FEngineSpec.MacKeySize, FEngineSpec.CipherKeySize);
  ...
  begin
    ...
    gen.Init(TECKeyGenerationParameters.Create(ecParams, FRandom) as IECKeyGenerationParameters);

    UsePointCompression := FEngineSpec.PointCompression;

    kGen := TEphemeralKeyPairGenerator.Create(gen, TKeyEncoder.Create(UsePointCompression) as IKeyEncoder);
    ...
    try
      ...
      on e: Exception do
      begin
        raise EBadBlockCryptoLibException.CreateResFmt(@SUnableToProcessBlock, [e.Message]);
      end;
    end;
  end
  else
  begin
    ...
    try
      FEngine.Init(Fkey, params, TECIESPublicKeyParser.Create(ecParams) as IECIESPublicKeyParser);
...
function TIESCipher.DoFinal(const input: TCryptoLibByteArray): TCryptoLibByteArray;
begin
  Result := DoFinal(input, 0, System.length(input));
end;

function TIESCipher.DoFinal(const input: TCryptoLibByteArray; inputOffset, inputLen: Int32; const output: TCryptoLibByteArray; outputOffset: Int32): Int32;
var
  buf: TCryptoLibByteArray;
begin
  buf := DoFinal(input, inputOffset, inputLen);
  System.Move(buf[0], output[outputOffset], System.length(buf) * System.SizeOf(Byte));
  Result := System.length(buf);
end;

procedure TIESCipher.Init(ForEncryption: Boolean; const Key: ICipherParameters; const EngineSpec: IAlgorithmParameterSpec; const Random: ISecureRandom);
var
...
begin
  ...
  begin
    raise EArgumentNilCryptoLibException.CreateRes(@SIAlgorithmParameterSpecNil);
  end
  ...
  begin
    raise EInvalidParameterCryptoLibException.CreateRes(@SIESParameterSpecError);
  end;

  Nonce := FEngineSpec.Nonce;

  if ((FivLength <> 0) and ((Nonce = nil) or (System.length(Nonce) <> FivLength))) then
  begin
    raise EInvalidParameterCryptoLibException.CreateResFmt(@SNonceInvalidLength, [FivLength]);
  end;
  ...
  begin
    if ((not Supports(LKey, IAsymmetricKeyParameter, Fkey)) or ((Fkey.IsPrivate))) then
    begin
  ...
  begin
    ...
    if ((not Supports(LKey, IAsymmetricKeyParameter, Fkey)) or (not(Fkey.IsPrivate))) then
    begin
...
procedure TIESCipher.ProcessBytes(const input: TCryptoLibByteArray; inputOffset, inputLen: Int32);
begin
  FBuffer.Write(input[inputOffset], inputLen);
end;
