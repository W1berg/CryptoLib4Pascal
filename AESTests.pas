...
type
  ...
  private
    ...
    procedure DoOidTest(const oids, names: TCryptoLibStringArray; groupSize: Int32);

    procedure DoAESTest(const cipher: IBufferedCipher; const param: ICipherParameters; const input, output: string; withpadding: Boolean = False);
...
procedure TTestAES.DoAESTest(const cipher: IBufferedCipher; const param: ICipherParameters; const input, output: string; withpadding: Boolean);
var
...
begin
  ...
  begin
    ...
    begin
      Fail(Format('Encryption Failed - Expected %s but got %s', [EncodeHex(LOutput), EncodeHex(EncryptionResult)]));
    end;
  end;
  ...
  begin
    Fail(Format('Decryption Failed - Expected %s but got %s', [EncodeHex(LInput), EncodeHex(DecryptionResult)]));
  end;
end;

procedure TTestAES.DoOidTest(const oids, names: TCryptoLibStringArray; groupSize: Int32);
var
...
begin
  ...
  for i := System.Low(TAESTestVectors.FOfficialVectorKeys_AES_CBC) to System.High(TAESTestVectors.FOfficialVectorKeys_AES_CBC) do
  begin
...
begin
  ...
  begin
    ...
    DoAESTest(cipher, KeyParametersWithIV as ICipherParameters, input, output, True);
  end;
...
begin
  ...
  for i := System.Low(TAESTestVectors.FOfficialVectorKeys_AES_CFB) to System.High(TAESTestVectors.FOfficialVectorKeys_AES_CFB) do
  begin
...
begin
  ...
  for i := System.Low(TAESTestVectors.FOfficialVectorKeys_AES_CTR) to System.High(TAESTestVectors.FOfficialVectorKeys_AES_CTR) do
  begin
...
begin
  ...
  for i := System.Low(TAESTestVectors.FOfficialVectorKeys_AES_ECB) to System.High(TAESTestVectors.FOfficialVectorKeys_AES_ECB) do
  begin
...
begin
  ...
  for i := System.Low(TAESTestVectors.FOfficialVectorKeys_AES_OFB) to System.High(TAESTestVectors.FOfficialVectorKeys_AES_OFB) do
  begin
...
begin
  oids := TCryptoLibStringArray.Create(TNistObjectIdentifiers.IdAes128Ecb.Id, TNistObjectIdentifiers.IdAes128Cbc.Id, TNistObjectIdentifiers.IdAes128Ofb.Id, TNistObjectIdentifiers.IdAes128Cfb.Id, TNistObjectIdentifiers.IdAes192Ecb.Id, TNistObjectIdentifiers.IdAes192Cbc.Id,
    TNistObjectIdentifiers.IdAes192Ofb.Id, TNistObjectIdentifiers.IdAes192Cfb.Id, TNistObjectIdentifiers.IdAes256Ecb.Id, TNistObjectIdentifiers.IdAes256Cbc.Id, TNistObjectIdentifiers.IdAes256Ofb.Id, TNistObjectIdentifiers.IdAes256Cfb.Id);

  names := TCryptoLibStringArray.Create('AES/ECB/PKCS7Padding', 'AES/CBC/PKCS7Padding', 'AES/OFB/NoPadding', 'AES/CFB/NoPadding', 'AES/ECB/PKCS7Padding', 'AES/CBC/PKCS7Padding', 'AES/OFB/NoPadding', 'AES/CFB/NoPadding', 'AES/ECB/PKCS7Padding', 'AES/CBC/PKCS7Padding', 'AES/OFB/NoPadding',
    'AES/CFB/NoPadding');
