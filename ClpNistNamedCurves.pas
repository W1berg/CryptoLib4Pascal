...
class procedure TNistNamedCurves.DefineCurveAlias(const name: string; const oid: IDerObjectIdentifier);
begin
  FobjIds.Add(UpperCase(name), oid);
  Fnames.Add(oid, name);
end;

class function TNistNamedCurves.GetByOid(const oid: IDerObjectIdentifier): IX9ECParameters;
begin
  result := TSecNamedCurves.GetByOid(oid);
end;

class function TNistNamedCurves.GetOid(const name: string): IDerObjectIdentifier;
begin
...
class function TNistNamedCurves.GetName(const oid: IDerObjectIdentifier): string;
begin
