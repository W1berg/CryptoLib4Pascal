...
resourcestring
  ...
  SOnlyFPCurvesAllowed = 'Only FP (Prime Field) Curves are Allowed for This Schnorr Implementation';
...
type
  ...
  /// </para>
  /// </summary>
  TECSchnorrSipaSigner = class sealed(TInterfacedObject, ISchnorrExt, IECSchnorrSipaSigner)
  ...
  var
    ...
    class procedure ValidateAllowedCurves(const ACurve: IECCurve); static; inline;
  ...
  public
    ...
    procedure Init(forSigning: Boolean; const parameters: ICipherParameters; const digest: IDigest); virtual;

    function GenerateSignature(const &message: TCryptoLibByteArray): TCryptoLibGenericArray<TBigInteger>; virtual;

    function VerifySignature(const &message: TCryptoLibByteArray; const RSig, SSig: TBigInteger): Boolean; virtual;
...
function TECSchnorrSipaSigner.GenerateSignature(const &message: TCryptoLibByteArray): TCryptoLibGenericArray<TBigInteger>;
var
...
begin
  ...
  begin
    ...
    raise EInvalidOperationCryptoLibException.CreateRes(@SNotInitializedForSigning);
  end;
  ...
  input := TArrayUtils.Concatenate(TBigIntegers.BigIntegerToBytes(PrivateKey, numBytes), &message);

  k := TBigInteger.Create(1, TDigestUtilities.DoFinal(FDigest, input)).&Mod(N);

  if k.CompareTo(TBigInteger.Zero) = 0 then
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SSignatureGenerationError);
  end;
  ...
  keyPrefixedM := TArrayUtils.Concatenate(TBigIntegers.BigIntegerToBytes(Xr, numBytes), TCryptoLibMatrixByteArray.Create(P.GetEncoded(true), &message));

  e := TBigInteger.Create(1, TDigestUtilities.DoFinal(FDigest, keyPrefixedM)).&Mod(N);
...
class procedure TECSchnorrSipaSigner.ValidateAllowedCurves(const ACurve: IECCurve);
begin
...
procedure TECSchnorrSipaSigner.Init(forSigning: Boolean; const parameters: ICipherParameters; const digest: IDigest);
var
...
function TECSchnorrSipaSigner.VerifySignature(const &message: TCryptoLibByteArray; const RSig, SSig: TBigInteger): Boolean;
var
  N, e: TBigInteger;
...
begin
  ...
  begin
    // not properly initialized... deal with it
    raise EInvalidOperationCryptoLibException.CreateRes(@SNotInitializedForVerifying);
  end;
  ...
  input := TArrayUtils.Concatenate(TBigIntegers.BigIntegerToBytes(RSig, numBytes), TCryptoLibMatrixByteArray.Create(PublicKeyBytes, &message));
  ...
  if ((r.IsInfinity) or (r.XCoord.ToBigInteger().CompareTo(RSig) <> 0) or (TBigInteger.Jacobi(r.YCoord.ToBigInteger(), PP) <> 1)) then
  begin
    result := false;
    Exit;
  end;
