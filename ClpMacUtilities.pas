...
type
  ...
  public
    ...
    class function GetMac(const id: IDerObjectIdentifier): IMac; overload; static; inline;
    ...
    class function CalculateMac(const algorithm: string; const cp: ICipherParameters; const input: TCryptoLibByteArray): TCryptoLibByteArray; static; inline;

    class function DoFinal(const mac: IMac): TCryptoLibByteArray; overload; static; inline;

    class function DoFinal(const mac: IMac; const input: TCryptoLibByteArray): TCryptoLibByteArray; overload; static; inline;
...
begin
  ...
  Falgorithms.Add(TNistObjectIdentifiers.IdHMacWithSha3_224.id, 'HMAC-SHA3-224');
  Falgorithms.Add(TNistObjectIdentifiers.IdHMacWithSha3_256.id, 'HMAC-SHA3-256');
  Falgorithms.Add(TNistObjectIdentifiers.IdHMacWithSha3_384.id, 'HMAC-SHA3-384');
  Falgorithms.Add(TNistObjectIdentifiers.IdHMacWithSha3_512.id, 'HMAC-SHA3-512');
...
{$ENDIF DELPHIXE3_UP}
    if ((TStringUtils.BeginsWith(mechanism, 'HMAC-', True)) or (TStringUtils.BeginsWith(mechanism, 'HMAC/', True))) then
    begin
  ...
  raise ESecurityUtilityCryptoLibException.CreateResFmt(@SUnRecognizedMac, [mechanism]);
end;
...
class function TMacUtilities.DoFinal(const mac: IMac; const input: TCryptoLibByteArray): TCryptoLibByteArray;
begin
  mac.BlockUpdate(input, 0, System.Length(input));
  result := DoFinal(mac);
end;

class function TMacUtilities.CalculateMac(const algorithm: string; const cp: ICipherParameters; const input: TCryptoLibByteArray): TCryptoLibByteArray;
var
  mac: IMac;
begin
...
class function TMacUtilities.GetAlgorithmName(const oid: IDerObjectIdentifier): string;
begin
