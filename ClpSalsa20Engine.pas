...
{$IFNDEF _FIXINSIGHT_}
  ...
  SInitError = '%s Init Parameters must Contain a KeyParameter (or null for Re-Init)';
  SKeyParameterNullForFirstInit = 'KeyParameter can not be null for First Initialisation';
{$ENDIF}
...
type
  ...
  const
    ...
    TAU_SIGMA: array [0 .. 7] of UInt32 = (1634760805, 824206446, 2036477238, 1797285236, 1634760805, 857760878, 2036477234, 1797285236);
  ...
  var
    ...
    class procedure PackTauOrSigma(keyLength: Int32; const state: TCryptoLibUInt32Array; stateOffset: Int32); static;
    class procedure SalsaCore(rounds: Int32; const input, x: TCryptoLibUInt32Array); static;
...
{$ENDIF}
    ...
    procedure ProcessBytes(const inBytes: TCryptoLibByteArray; inOff, len: Int32; const outBytes: TCryptoLibByteArray; outOff: Int32); virtual;
...
begin
  ...
  TConverters.le32_copy(PCardinal(Fx), 0, PByte(output), 0, System.Length(Fx) * System.SizeOf(UInt32));
end;
...
procedure TSalsa20Engine.Init(forEncryption: Boolean; const parameters: ICipherParameters);
var
...
begin
  ...
  *)
  ...
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SIVRequired, [AlgorithmName]);
  end;
  ...
  begin
    ...
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidIV, [AlgorithmName, NonceSize]);
  end;
  ...
  begin
    ...
    begin
      raise EArgumentCryptoLibException.CreateResFmt(@SKeyParameterNullForFirstInit, [AlgorithmName]);
    end;
  ...
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SInitError, [AlgorithmName]);
  end;
...
class procedure TSalsa20Engine.PackTauOrSigma(keyLength: Int32; const state: TCryptoLibUInt32Array; stateOffset: Int32);
var
  tsOff: Int32;
begin
...
procedure TSalsa20Engine.ProcessBytes(const inBytes: TCryptoLibByteArray; inOff, len: Int32; const outBytes: TCryptoLibByteArray; outOff: Int32);
var
  Idx: Int32;
begin
  if (not FInitialised) then
  begin
    raise EInvalidOperationCryptoLibException.CreateResFmt(@SEngineNotInitialized, [AlgorithmName]);
  end;
...
class procedure TSalsa20Engine.SalsaCore(rounds: Int32; const input, x: TCryptoLibUInt32Array);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      raise EArgumentCryptoLibException.CreateResFmt(@SInvalidKeySize, [AlgorithmName]);
    end;
    ...
    // Key
    TConverters.le32_copy(PByte(keyBytes), 0, PCardinal(FEngineState), 1 * System.SizeOf(UInt32), 4 * System.SizeOf(UInt32));
    TConverters.le32_copy(PByte(keyBytes), (System.Length(keyBytes) - 16) * System.SizeOf(Byte), PCardinal(FEngineState), 11 * System.SizeOf(UInt32), 4 * System.SizeOf(UInt32));
  end;

  // IV
  TConverters.le32_copy(PByte(ivBytes), 0, PCardinal(FEngineState), 6 * System.SizeOf(UInt32), 2 * System.SizeOf(UInt32));
