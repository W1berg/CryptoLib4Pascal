...
begin
  ...
  Fvalues := TCryptoLibGenericArray<IAsn1Object>.Create(
    ...
    TDerBoolean.True, TDerBoolean.False, TDerEnumerated.Create(100), TDerBmpString.Create('hello world'),

    TDerGeneralString.Create('hello world'), TDerIA5String.Create('hello'), TDerInteger.Create(1000), TDerNull.Instance, TDerNumericString.Create('123456'), TDerObjectIdentifier.Create('1.1.1.10000.1'), TDerOctetString.Create(Fdata), TDerPrintableString.Create('hello world'),

    TDerT61String.Create('hello world'),

    TDerUniversalString.Create(Fdata),

    TDerUtf8String.Create('hello world'), TDerVisibleString.Create('hello world'), TDerGraphicString.Create(DecodeHex('deadbeef')), TDerVideotexString.Create(TConverters.ConvertStringToBytes('Hello World', TEncoding.ANSI)),
    ...
    TBerSequence.Create(TDerPrintableString.Create('hello world') as IDerPrintableString), TBerSet.Create(TDerPrintableString.Create('hello world') as IDerPrintableString), TDerSequence.Create(TDerPrintableString.Create('hello world') as IDerPrintableString),
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          Fail(Format('Failed equality test for %s', [(o as TAsn1Object).ClassName]));
        end;

        if (not(o.GetHashCode() = Fvalues[i].GetHashCode())) then
        begin
          Fail(Format('Failed hashCode test for %s', [(o as TAsn1Object).ClassName]));
        end;
