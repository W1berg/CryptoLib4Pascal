...
resourcestring
  ...
  SSICModeWarning = 'Warning: SIC-Mode Can Become a TwoTime-Pad if the Blocksize of the Cipher is Too Small. Use a Cipher With a Block Size of at Least 128 bits (e.g. AES)';
  SModeAndPaddingNotNeededStreamCipher = 'Modes and Paddings Not Used for Stream Ciphers';
...
{$SCOPEDENUMS ON}
    ...
    TCipherPadding = (NOPADDING, ISO10126PADDING, ISO10126D2PADDING, ISO10126_2PADDING, ISO7816_4PADDING, ISO9797_1PADDING, PKCS5, PKCS5PADDING, PKCS7, PKCS7PADDING, TBCPADDING, WITHCTS, X923PADDING, ZEROBYTEPADDING);
{$SCOPEDENUMS OFF}
  ...
  public
    ...
    class function GetCipher(algorithm: string): IBufferedCipher; overload; static;
    class function GetCipher(const oid: IDerObjectIdentifier): IBufferedCipher; overload; static; inline;
...
begin
  ...
  Falgorithms.Add(TNistObjectIdentifiers.IdAes128Ecb.Id, 'AES/ECB/PKCS7PADDING');
  Falgorithms.Add(TNistObjectIdentifiers.IdAes192Ecb.Id, 'AES/ECB/PKCS7PADDING');
  Falgorithms.Add(TNistObjectIdentifiers.IdAes256Ecb.Id, 'AES/ECB/PKCS7PADDING');
  ...
  Falgorithms.Add(TNistObjectIdentifiers.IdAes128Cbc.Id, 'AES/CBC/PKCS7PADDING');
  Falgorithms.Add(TNistObjectIdentifiers.IdAes192Cbc.Id, 'AES/CBC/PKCS7PADDING');
  Falgorithms.Add(TNistObjectIdentifiers.IdAes256Cbc.Id, 'AES/CBC/PKCS7PADDING');
...
begin
  ...
  cipherAlgorithm := TCipherAlgorithm(GetEnumValue(TypeInfo(TCipherAlgorithm), temp));
  ...
  case cipherAlgorithm of
    ...
    begin
      raise ESecurityUtilityCryptoLibException.CreateResFmt(@SUnRecognizedCipher, [algorithm]);
    end;
  end;
  ...
  begin
    ...
    Result := TBufferedStreamCipher.Create(streamCipher) as IBufferedStreamCipher;
    Exit;
  end;
  ...
  begin
    ...
    cipherPadding := TCipherPadding(GetEnumValue(TypeInfo(TCipherPadding), temp));
...
{$ENDIF DELPHIXE3_UP}
          ...
          blockCipher := TCfbBlockCipher.Create(blockCipher, bits) as ICfbBlockCipher;
        end;
...
{$ENDIF DELPHIXE3_UP}
          ...
          blockCipher := TOfbBlockCipher.Create(blockCipher, bits) as IOfbBlockCipher;
        end;
  ...
  begin
    ...
    if (padding <> nil) then
    begin
      Result := TPaddedBufferedBlockCipher.Create(blockCipher, padding) as IPaddedBufferedBlockCipher;
      Exit;
    end;

    if ((not padded) or (blockCipher.IsPartialBlockOkay)) then
    begin
      Result := TBufferedBlockCipher.Create(blockCipher) as IBufferedBlockCipher;
      Exit;
    end;

    Result := TPaddedBufferedBlockCipher.Create(blockCipher) as IPaddedBufferedBlockCipher;
    Exit;
  end;
...
class function TCipherUtilities.GetCipher(const oid: IDerObjectIdentifier): IBufferedCipher;
begin
  Result := GetCipher(oid.Id);
end;

class function TCipherUtilities.GetObjectIdentifier(mechanism: string): IDerObjectIdentifier;
var
  aliased: string;
begin
  if (mechanism = '') then
