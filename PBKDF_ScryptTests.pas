...
function TTestPBKDF_Scrypt.DoTestVector(const APassword, ASalt: string; ACost, ABlockSize, AParallelism, AOutputSize: Int32): string;
var
...
begin
  ...
  PBKDF_Scrypt := TKDF.TPBKDF_Scrypt.CreatePBKDF_Scrypt(APasswordBytes, ASaltBytes, ACost, ABlockSize, AParallelism);
  OutputBytes := PBKDF_Scrypt.GetBytes(AOutputSize);
  PBKDF_Scrypt.Clear();
  Result := TConverters.ConvertBytesToHexString(OutputBytes, False);
end;

procedure TTestPBKDF_Scrypt.DoCheckIllegal(const AMsg: string; const APassword, ASalt: TBytes; ACost, ABlockSize, AParallelism, AOutputSize: Int32);
begin
  try
    TKDF.TPBKDF_Scrypt.CreatePBKDF_Scrypt(APassword, ASalt, ACost, ABlockSize, AParallelism).GetBytes(AOutputSize);
    Fail(AMsg);
  except
...
procedure TTestPBKDF_Scrypt.DoCheckOk(const AMsg: string; const APassword, ASalt: TBytes; ACost, ABlockSize, AParallelism, AOutputSize: Int32);
var
  PBKDF_Scrypt: IPBKDF_Scrypt;
begin
  try
    try
      PBKDF_Scrypt := TKDF.TPBKDF_Scrypt.CreatePBKDF_Scrypt(APassword, ASalt, ACost, ABlockSize, AParallelism);
...
begin
  ...
  DoCheckIllegal('Cost parameter must < 65536 for r = 1', nil, nil, 65536, 1, 1, 1);
  DoCheckIllegal('Block size must be >= 1', nil, nil, 2, 0, 2, 1);
  DoCheckIllegal('Parallelisation parameter must be >= 1', nil, nil, 2, 1, 0, 1);
  // disabled test because it's very expensive
  // DoCheckOk('Parallelisation parameter 65535 OK for r = 4', Nil, Nil, 2, 32,
  // 65535, 1);
  DoCheckIllegal('Parallelisation parameter must be < 65535 for r = 4', nil, nil, 2, 32, 65536, 1);
