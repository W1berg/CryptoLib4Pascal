...
resourcestring
  SInvalidByteCount = '"(AByteCount)" Argument must be a value greater than zero.';
  ...
  SInvalidParallelism = 'Parallelism parameter must be >= 1 and <= %d (based on block size of %d)';
...
type
  ...
  /// </remarks>
  TPBKDF_ScryptNotBuildInAdapter = class sealed(TKDF, IPBKDF_Scrypt, IPBKDF_ScryptNotBuildIn)
  ...
  var
    ...
    class procedure ClearArray(const AInput: THashLibByteArray); overload; static; inline;
    class procedure ClearArray(const AInput: THashLibUInt32Array); overload; static; inline;

    class procedure ClearAllArrays(const AInputs: THashLibMatrixUInt32Array); static;

    class function IsPowerOf2(Ax: Int32): Boolean; static; inline;

    class function SingleIterationPBKDF2(const APasswordBytes, ASaltBytes: THashLibByteArray; AOutputLength: Int32): THashLibByteArray; static;
    ...
    /// rotated AValue
    /// </returns>
    class function Rotl(AValue: UInt32; ADistance: Int32): UInt32; static; inline;

    /// <summary>
    /// lifted from <c>ClpSalsa20Engine.pas</c> in CryptoLib4Pascal with
    /// minor modifications.
    /// </summary>
    class procedure SalsaCore(ARounds: Int32; const AInput, Ax: THashLibUInt32Array); static;

    class procedure &Xor(const Aa, Ab: THashLibUInt32Array; AbOff: Int32; const AOutput: THashLibUInt32Array); static;

    class procedure SMix(AIdx: Int32; APtrDataContainer: PDataContainer); static;

    class procedure BlockMix(const Ab, AX1, AX2, Ay: THashLibUInt32Array; AR: Int32); static;

    class procedure DoParallelSMix(ADataContainer: PDataContainer); static;

{$IFDEF USE_PASMP}
    class procedure PasMPSMixWrapper(const AJob: PPasMPJob; const AThreadIndex: LongInt; const ADataContainer: Pointer; const AFromIndex, AToIndex: TPasMPNativeInt); inline;
{$ENDIF USE_PASMP}
{$IFDEF USE_MTPROCS}
    class procedure MTProcsSMixWrapper(AIdx: PtrInt; ADataContainer: Pointer; AItem: TMultiThreadProcItem); inline;
{$ENDIF USE_MTPROCS}
    class function MFCrypt(const APasswordBytes, ASaltBytes: THashLibByteArray; ACost, ABlockSize, AParallelism, AOutputLength: Int32): THashLibByteArray; static;

  public

    class procedure ValidatePBKDF_ScryptInputs(ACost, ABlockSize, AParallelism: Int32); static;

    constructor Create(const APasswordBytes, ASaltBytes: THashLibByteArray; ACost, ABlockSize, AParallelism: Int32);
...
class procedure TPBKDF_ScryptNotBuildInAdapter.ClearArray(const AInput: THashLibByteArray);
begin
  TArrayUtils.ZeroFill(AInput);
end;

class procedure TPBKDF_ScryptNotBuildInAdapter.ClearArray(const AInput: THashLibUInt32Array);
begin
  TArrayUtils.ZeroFill(AInput);
end;

class procedure TPBKDF_ScryptNotBuildInAdapter.ClearAllArrays(const AInputs: THashLibMatrixUInt32Array);
var
  LIdx: Int32;
begin
...
class function TPBKDF_ScryptNotBuildInAdapter.SingleIterationPBKDF2(const APasswordBytes, ASaltBytes: THashLibByteArray; AOutputLength: Int32): THashLibByteArray;
begin
  result := (TPBKDF2_HMACNotBuildInAdapter.Create(TSHA2_256.Create() as IHash, APasswordBytes, ASaltBytes, 1) as IPBKDF2_HMAC).GetBytes(AOutputLength);
end;

class procedure TPBKDF_ScryptNotBuildInAdapter.&Xor(const Aa, Ab: THashLibUInt32Array; AbOff: Int32; const AOutput: THashLibUInt32Array);
var
  LIdx: Int32;
begin
...
class function TPBKDF_ScryptNotBuildInAdapter.Rotl(AValue: UInt32; ADistance: Int32): UInt32;
begin
  result := TBits.RotateLeft32(AValue, ADistance);
end;

class procedure TPBKDF_ScryptNotBuildInAdapter.SalsaCore(ARounds: Int32; const AInput, Ax: THashLibUInt32Array);
var
  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13, x14, x15: UInt32;
  LIdx: Int32;
begin
...
class procedure TPBKDF_ScryptNotBuildInAdapter.BlockMix(const Ab, AX1, AX2, Ay: THashLibUInt32Array; AR: Int32);
var
...
class procedure TPBKDF_ScryptNotBuildInAdapter.SMix(AIdx: Int32; APtrDataContainer: PDataContainer);
var
...
begin
  ...
  try
    ...
    begin
      ...
      System.Move(LV[LJdx * LBCount], LBlockY[0], LBCount * System.SizeOf(UInt32));
    ...
    ClearArray(LV);
    ClearAllArrays(THashLibMatrixUInt32Array.Create(LX, LBlockX1, LBlockX2, LBlockY));
  end;
end;

{$IFDEF USE_PASMP}

class procedure TPBKDF_ScryptNotBuildInAdapter.PasMPSMixWrapper(const AJob: PPasMPJob; const AThreadIndex: LongInt; const ADataContainer: Pointer; const AFromIndex, AToIndex: TPasMPNativeInt);
begin
...
class procedure TPBKDF_ScryptNotBuildInAdapter.MTProcsSMixWrapper(AIdx: PtrInt; ADataContainer: Pointer; AItem: TMultiThreadProcItem);
begin
...
class procedure TPBKDF_ScryptNotBuildInAdapter.DoParallelSMix(ADataContainer: PDataContainer);
...
{$IF DEFINED(USE_PASMP)}
  ...
  GlobalPasMP.Invoke(GlobalPasMP.ParallelFor(ADataContainer, 0, LParallelism - 1, PasMPSMixWrapper));
{$ELSEIF DEFINED(USE_MTPROCS)}
  ProcThreadPool.DoParallel(MTProcsSMixWrapper, 0, LParallelism - 1, ADataContainer);
{$ELSE}
...
class function TPBKDF_ScryptNotBuildInAdapter.MFCrypt(const APasswordBytes, ASaltBytes: THashLibByteArray; ACost, ABlockSize, AParallelism, AOutputLength: Int32): THashLibByteArray;
var
...
begin
  ...
  LBytes := SingleIterationPBKDF2(APasswordBytes, ASaltBytes, AParallelism * LMFLenBytes);

  try
    LBLen := System.Length(LBytes) div 4;
    System.SetLength(Lb, LBLen);

    TConverters.le32_copy(PByte(LBytes), 0, PCardinal(Lb), 0, System.Length(LBytes) * System.SizeOf(Byte));
    ...
    TConverters.le32_copy(PCardinal(Lb), 0, PByte(LBytes), 0, System.Length(Lb) * System.SizeOf(UInt32));
...
class procedure TPBKDF_ScryptNotBuildInAdapter.ValidatePBKDF_ScryptInputs(ACost, ABlockSize, AParallelism: Int32);
var
  LMaxParallel: Int32;
begin
  ...
  begin
    raise EArgumentHashLibException.CreateResFmt(@SInvalidParallelism, [LMaxParallel, ABlockSize]);
  end;
end;
...
constructor TPBKDF_ScryptNotBuildInAdapter.Create(const APasswordBytes, ASaltBytes: THashLibByteArray; ACost, ABlockSize, AParallelism: Int32);
begin
  inherited Create();
...
function TPBKDF_ScryptNotBuildInAdapter.GetBytes(AByteCount: Int32): THashLibByteArray;
begin
  ...
  result := MFCrypt(FPasswordBytes, FSaltBytes, FCost, FBlockSize, FParallelism, AByteCount);
end;
