...
type
  ...
  /// </para>
  /// </summary>
  TDsaKeyPairGenerator = class sealed(TInterfacedObject, IAsymmetricCipherKeyPairGenerator, IDsaKeyPairGenerator)
  ...
  var
    ...
    class function GeneratePrivateKey(const q: TBigInteger; const random: ISecureRandom): TBigInteger; static;

    class function CalculatePublicKey(const p, g, x: TBigInteger): TBigInteger; static; inline;
...
class function TDsaKeyPairGenerator.CalculatePublicKey(const p, g, x: TBigInteger): TBigInteger;
begin
  result := g.ModPow(x, p);
end;
...
begin
  ...
  result := TAsymmetricCipherKeyPair.Create(TDsaPublicKeyParameters.Create(y, dsaParams) as IDsaPublicKeyParameters, TDsaPrivateKeyParameters.Create(x, dsaParams) as IDsaPrivateKeyParameters);
end;

class function TDsaKeyPairGenerator.GeneratePrivateKey(const q: TBigInteger; const random: ISecureRandom): TBigInteger;
var
  minWeight: Int32;
  x, One: TBigInteger;
begin
