...
type
  ...
  var
    ...
    class function GetNext(const e: TCryptoLibGenericArray<IAsn1Encodable>; var Idx: Int32): IAsn1Encodable; static; inline;

  public
    constructor Create(const p, g, q, j: IDerInteger; const validationParams: IDHValidationParams); overload;
    ...
    class function GetInstance(const obj: IAsn1TaggedObject; isExplicit: Boolean): IDHDomainParameters; overload; static; inline;

    class function GetInstance(obj: TObject): IDHDomainParameters; overload; static; inline;
...
class function TDHDomainParameters.GetNext(const e: TCryptoLibGenericArray<IAsn1Encodable>; var Idx: Int32): IAsn1Encodable;
begin
...
begin
  ...
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SBadSequenceSize, [seq.Count]);
  end;
  ...
  if (next <> nil) then
  begin
    FvalidationParams := TDHValidationParams.GetInstance(next.ToAsn1Object() as TAsn1Object);
  end;

end;

constructor TDHDomainParameters.Create(const p, g, q, j: IDerInteger; const validationParams: IDHValidationParams);
begin
  inherited Create();
...
class function TDHDomainParameters.GetInstance(obj: TObject): IDHDomainParameters;
begin
  ...
  raise EArgumentCryptoLibException.CreateResFmt(@SInvalidDHDomainParameters, [obj.ToString]);
end;

class function TDHDomainParameters.GetInstance(const obj: IAsn1TaggedObject; isExplicit: Boolean): IDHDomainParameters;
begin
  result := GetInstance(TAsn1Sequence.GetInstance(obj, isExplicit) as TAsn1Sequence);
end;
