...
type
  ...
  const
    ...
    class function InternalEncode(const bytes: TSimpleBaseLibByteArray; const alphabet: string): string; static;

    class function Decode(const text: TSimpleBaseLibCharArray): TSimpleBaseLibByteArray; overload; static;
  ...
  public
    ...
    class function EncodeUpper(const bytes: TSimpleBaseLibByteArray): string; static; inline;
    ...
    /// <param name="bytes">Bytes to encode</param>
    /// <returns>Base16 string</returns>
    class function EncodeLower(const bytes: TSimpleBaseLibByteArray): string; static; inline;

    class function Decode(const text: string): TSimpleBaseLibByteArray; overload; static;
...
class function TBase16.Decode(const text: TSimpleBaseLibCharArray): TSimpleBaseLibByteArray;
  ...
  begin
    case c of
      ...
      begin
        raise EArgumentSimpleBaseLibException.CreateResFmt(@SInvalidHexCharacter, [Char(c)]);
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    raise EArgumentSimpleBaseLibException.CreateResFmt(@SInvalidTextLength, [Ltext]);
  end;
...
class function TBase16.InternalEncode(const bytes: TSimpleBaseLibByteArray; const alphabet: string): string;
var
  bytesLen: Int32;
  b: Byte;
...
class function TBase16.EncodeLower(const bytes: TSimpleBaseLibByteArray): string;
begin
  Result := InternalEncode(bytes, lowerAlphabet);
end;

class function TBase16.EncodeUpper(const bytes: TSimpleBaseLibByteArray): string;
begin
