...
resourcestring
  ...
  SNotInitializedForVerifyingRecovery = 'Not Initialised For Verifying/Recovery';
...
type
  ...
  var
    ...
    function ExtractT(const pubKey: IECPublicKeyParameters; const r, s: TBigInteger): TBigInteger;
...
function TECNRSigner.ExtractT(const pubKey: IECPublicKeyParameters; const r, s: TBigInteger): TBigInteger;
var
  n, x: TBigInteger;
  G, W, P: IECPoint;
begin
...
function TECNRSigner.GenerateSignature(const &message: TCryptoLibByteArray): TCryptoLibGenericArray<TBigInteger>;
var
...
begin
  ...
  repeat // generate r
    ...
    keyGen.Init(TECKeyGenerationParameters.Create(privKey.parameters, FRandom) as IECKeyGenerationParameters);
...
procedure TECNRSigner.Init(forSigning: Boolean; const parameters: ICipherParameters);
var
...
function TECNRSigner.VerifySignature(const &message: TCryptoLibByteArray; const r, s: TBigInteger): Boolean;
var
...
function TECNRSigner.GetRecoveredMessage(const r, s: TBigInteger): TCryptoLibByteArray;
var
  t: TBigInteger;
begin
  if (FForSigning) then
  begin
    raise EInvalidOperationCryptoLibException.CreateRes(@SNotInitializedForVerifyingRecovery);
  end;
