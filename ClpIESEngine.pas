...
resourcestring
  SErrorRecoveringEphemeralPublicKey = 'Unable to Recover Ephemeral Public Key: "%s"';
  SInvalidCipherTextLength = 'Length of Input Must be Greater than the MAC and V Combined';
...
type
  ...
  var
    ...
    function GetLengthTag(const p2: TCryptoLibByteArray): TCryptoLibByteArray; inline;

    procedure ExtractParams(const params: ICipherParameters); inline;

    function SimilarMacCompute(const ArgOne, ArgTwo: TCryptoLibByteArray): TCryptoLibByteArray; inline;
  ...
  var
    ...
    procedure SetupBlockCipherAndMacKeyBytes(out K1, K2: TCryptoLibByteArray); inline;

    function EncryptBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray; virtual;

    function DecryptBlock(const in_enc: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray; virtual;
  ...
  public
    ...
    /// </param>
    constructor Create(const agree: IBasicAgreement; const kdf: IDerivationFunction; const mac: IMac); overload;
    ...
    /// </param>
    constructor Create(const agree: IBasicAgreement; const kdf: IDerivationFunction; const mac: IMac; const cipher: IBufferedBlockCipher); overload;
    ...
    /// </param>
    procedure Init(forEncryption: Boolean; const privParam, pubParam, params: ICipherParameters); overload;
    ...
    /// </param>
    procedure Init(const publicKey: IAsymmetricKeyParameter; const params: ICipherParameters; const ephemeralKeyPairGenerator: IEphemeralKeyPairGenerator); overload;
    ...
    /// </param>
    procedure Init(const privateKey: IAsymmetricKeyParameter; const params: ICipherParameters; const publicKeyParser: IKeyParser); overload;

    function ProcessBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray; virtual;
...
function TIESEngine.GetLengthTag(const p2: TCryptoLibByteArray): TCryptoLibByteArray;
begin
...
function TIESEngine.SimilarMacCompute(const ArgOne, ArgTwo: TCryptoLibByteArray): TCryptoLibByteArray;
begin
  if (ArgOne <> nil) then
  begin
...
procedure TIESEngine.SetupBlockCipherAndMacKeyBytes(out K1, K2: TCryptoLibByteArray);
var
  K: TCryptoLibByteArray;
begin
  System.SetLength(K1, (Fparam as IIESWithCipherParameters).CipherKeySize div 8);
  ...
  System.Move(K[System.Length(K1)], K2[0], System.Length(K2) * System.SizeOf(Byte));
end;

constructor TIESEngine.Create(const agree: IBasicAgreement; const kdf: IDerivationFunction; const mac: IMac);
begin
  inherited Create();
  Fagree := agree;
  Fkdf := kdf;
...
constructor TIESEngine.Create(const agree: IBasicAgreement; const kdf: IDerivationFunction; const mac: IMac; const cipher: IBufferedBlockCipher);
begin
  inherited Create();
  Fagree := agree;
  Fkdf := kdf;
...
function TIESEngine.DecryptBlock(const in_enc: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  begin
    raise EInvalidCipherTextCryptoLibException.CreateRes(@SInvalidCipherTextLength);
  end;
  ...
  begin
    ...
    begin
      ...
      if K1 <> nil then
      begin
        System.Move(K[System.Length(K2)], K1[0], System.Length(K1) * System.SizeOf(Byte));
      end;
    end
    else
    begin
  ...
  begin
    ...
    System.SetLength(M, Fcipher.GetOutputSize(inLen - System.Length(FV) - Fmac.GetMacSize));

    // do initial processing
    len := Fcipher.ProcessBytes(in_enc, inOff + System.Length(FV), inLen - System.Length(FV) - Fmac.GetMacSize, M, 0);
  ...
  Fmac.BlockUpdate(in_enc, inOff + System.Length(FV), inLen - System.Length(FV) - System.Length(T2));
...
function TIESEngine.EncryptBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  begin
    ...
    if (FIV <> nil) then
    begin
      Fcipher.Init(true, TParametersWithIV.Create(TKeyParameter.Create(K1) as IKeyParameter, FIV) as IParametersWithIV);
    end
    else
    begin
  ...
  System.Move(T[0], Result[System.Length(FV) + len], System.Length(T) * System.SizeOf(Byte));
...
procedure TIESEngine.Init(const privateKey: IAsymmetricKeyParameter; const params: ICipherParameters; const publicKeyParser: IKeyParser);
begin
...
procedure TIESEngine.Init(const publicKey: IAsymmetricKeyParameter; const params: ICipherParameters; const ephemeralKeyPairGenerator: IEphemeralKeyPairGenerator);
begin
  FforEncryption := true;
  FpubParam := publicKey;
...
procedure TIESEngine.Init(forEncryption: Boolean; const privParam, pubParam, params: ICipherParameters);
begin
...
function TIESEngine.ProcessBlock(const &in: TCryptoLibByteArray; inOff, inLen: Int32): TCryptoLibByteArray;
var
    ...
    begin
      ...
      try
        ...
        try
          ...
          begin
            raise EInvalidCipherTextCryptoLibException.CreateResFmt(@SErrorRecoveringEphemeralPublicKey, [e.Message]);
          end;
