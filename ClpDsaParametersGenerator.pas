...
resourcestring
  SInvalidLValue = 'L Values Must be Between 1024 and 3072 and a Multiple of 1024';
  SInvalidNValueForSpecifiedL = 'N Must be " %d " for L = " %d "';
  SInvalidNValueForSpecifiedL_Two = 'N Must be " %d " or " %d " for L = " %d "';
  SDigestOutputSizeTooSmallForN = 'Digest Output Size Too Small for Value of N Which is " %d "';
  SUnsupportedDigest = 'Can Only Use SHA-1 For Generating FIPS 186-2 Parameters';
  SInvalidDsaKeyStrength = 'Size Must Be From %d - %d and a multiple of %d, "%d"';
...
type
  ...
  strict private
    ...
    class procedure Hash(const d: IDigest; const input, output: TCryptoLibByteArray; outputPos: Int32); static; inline;

    class procedure Inc(const buf: TCryptoLibByteArray); static; inline;

    class function CalculateGenerator_FIPS186_2(const p, q: TBigInteger; const r: ISecureRandom): TBigInteger; static; inline;

    class function CalculateGenerator_FIPS186_3_Unverifiable(const p, q: TBigInteger; const r: ISecureRandom): TBigInteger; static; inline;

    class function CalculateGenerator_FIPS186_3_Verifiable(const d: IDigest; const p, q: TBigInteger; const seed: TCryptoLibByteArray; index: Int32): TBigInteger; static; inline;
  ...
  public
    ...
    /// random byte source.
    /// </param>
    procedure Init(size, certainty: Int32; const random: ISecureRandom); overload;
    ...
    /// random byte source.
    /// </param>
    procedure Init(size, certainty, iterations: Int32; const random: ISecureRandom); overload;
...
class function TDsaParametersGenerator.IsValidDsaStrength(strength: Int32): Boolean;
begin
  // result := (strength >= 512) and (strength <= 1024) and ((strength mod 64) = 0);
  result := (strength >= 512) and (strength <= 1024) and ((strength and 63) = 0);
end;
...
class procedure TDsaParametersGenerator.Hash(const d: IDigest; const input, output: TCryptoLibByteArray; outputPos: Int32);
begin
...
class function TDsaParametersGenerator.CalculateGenerator_FIPS186_2(const p, q: TBigInteger; const r: ISecureRandom): TBigInteger;
var
...
class function TDsaParametersGenerator.CalculateGenerator_FIPS186_3_Unverifiable(const p, q: TBigInteger; const r: ISecureRandom): TBigInteger;
begin
  result := CalculateGenerator_FIPS186_2(p, q, r);
end;

class function TDsaParametersGenerator.CalculateGenerator_FIPS186_3_Verifiable(const d: IDigest; const p, q: TBigInteger; const seed: TCryptoLibByteArray; index: Int32): TBigInteger;
var
  e, W, g: TBigInteger;
...
begin
  ...
  System.Move(ggen[0], U[System.Length(seed)], System.Length(ggen) * System.SizeOf(Byte));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        result := TDsaParameters.Create(p, q, g, TDsaValidationParameters.Create(seed, counter) as IDsaValidationParameters);
        Exit;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          g := CalculateGenerator_FIPS186_3_Verifiable(d, p, q, seed, FusageIndex);
          if (g.IsInitialized) then
          begin
            result := TDsaParameters.Create(p, q, g, TDsaValidationParameters.Create(seed, counter, FusageIndex) as IDsaValidationParameters);
            Exit;
          end;
        end;
...
procedure TDsaParametersGenerator.Init(size, certainty: Int32; const random: ISecureRandom);
begin
  Init(size, certainty, Max(GetMinimumIterations(size), (certainty + 1) div 2), random);
end;

procedure TDsaParametersGenerator.Init(size, certainty, iterations: Int32; const random: ISecureRandom);
begin
  if (not IsValidDsaStrength(size)) then
  begin
    raise EArgumentCryptoLibException.CreateResFmt(@SInvalidDsaKeyStrength, [512, 1024, 64, size]);
  end;
  FL := size;
  FN := GetDefaultN(size);
...
procedure TDsaParametersGenerator.Init(const params: IDsaParameterGenerationParameters);
var
  L, n: Int32;
begin
  L := params.L;
  ...
  begin
    raise EInvalidParameterCryptoLibException.CreateResFmt(@SInvalidNValueForSpecifiedL, [160, 1024]);
  end
  else if ((L = 2048) and ((n <> 224) and (n <> 256))) then
  begin
    raise EInvalidParameterCryptoLibException.CreateResFmt(@SInvalidNValueForSpecifiedL_Two, [224, 256, 2048]);
  end
  else if ((L = 3072) and (n <> 256)) then
  begin
    raise EInvalidParameterCryptoLibException.CreateResFmt(@SInvalidNValueForSpecifiedL, [256, 3072]);
  end;

  if ((Fdigest.GetDigestSize * 8) < n) then
  begin
    raise EInvalidParameterCryptoLibException.CreateResFmt(@SDigestOutputSizeTooSmallForN, [n]);
  end;
