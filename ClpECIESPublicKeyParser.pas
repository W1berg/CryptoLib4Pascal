...
resourcestring
  ...
  SSenderPublicKeyInvalidPointEncoding = 'Sender''s Public Key has Invalid Point Encoding "%x"';

type
  TECIESPublicKeyParser = class sealed(TInterfacedObject, IECIESPublicKeyParser, IKeyParser)
...
function TECIESPublicKeyParser.ReadKey(const Stream: TStream): IAsymmetricKeyParameter;
var
...
begin
  ...
  case first of
    ...
    begin
      raise EIOCryptoLibException.CreateResFmt(@SSenderPublicKeyInvalidPointEncoding, [first]);
    end;
  ...
  result := TECPublicKeyParameters.Create(FecParams.Curve.DecodePoint(v), FecParams);
end;
