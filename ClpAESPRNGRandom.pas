...
resourcestring
  SInvalidAESRNGSeedLength = 'AES RNG Seed Length must be either one of these "128/192/256 bits".';
...
type
  ...
  public
    ...
    class procedure GetNonZeroBytes(const data: TCryptoLibByteArray); overload; static;
...
begin
  if ((ASeedLength < 16) or (ASeedLength > 32) or ((ASeedLength and 7) <> 0)) then
  begin
    raise EArgumentCryptoLibException.CreateRes(@SInvalidAESRNGSeedLength);
  end;
end;

class procedure TAESPRNGRandom.GetRawEntropy(const AEntropy: TCryptoLibByteArray);
begin
...
constructor TAESPRNGRandom.Create(const AAESRNGSeed: TCryptoLibByteArray; AReseedAfterBytes: Int32);
var
