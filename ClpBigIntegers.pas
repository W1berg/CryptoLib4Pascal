...
type
  ...
  public
    ...
    /// encoding.
    /// </returns>
    class function AsUnsignedByteArray(const n: TBigInteger): TCryptoLibByteArray; overload; static;
    ...
    /// given the size of n.
    /// </returns>
    class function AsUnsignedByteArray(length: Int32; const n: TBigInteger): TCryptoLibByteArray; overload; static;
    ...
    /// </returns>
    class function BigIntegerToBytes(const b: TBigInteger; numBytes: Int32): TCryptoLibByteArray; static; inline;

    class function GetUnsignedByteLength(const n: TBigInteger): Int32; static; inline;
...
class function TBigIntegers.AsUnsignedByteArray(const n: TBigInteger): TCryptoLibByteArray;
begin
  Result := n.ToByteArrayUnsigned();
end;

class function TBigIntegers.AsUnsignedByteArray(length: Int32; const n: TBigInteger): TCryptoLibByteArray;
var
...
begin
  ...
  System.Move(bytes[0], Result[System.length(Result) - System.length(bytes)], System.length(bytes) * System.SizeOf(Byte));

end;

class function TBigIntegers.BigIntegerToBytes(const b: TBigInteger; numBytes: Int32): TCryptoLibByteArray;
var
...
begin
  ...
  System.Move(biBytes[start], Result[numBytes - length], length * System.SizeOf(Byte));
end;
...
begin
  ...
  begin
    Result := CreateRandomInRange(TBigInteger.Zero, max.Subtract(min), random).Add(min);
    Exit;
  end;
  ...
  Result := TBigInteger.Create(max.Subtract(min).BitLength - 1, random).Add(min);
end;
